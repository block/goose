        ...
        with open(session_file, "r") as f:
            for line in f:
                try:
                    ...
                    if "content" in message_obj and isinstance(
                        ...
                        for content_item in message_obj["content"]:
                            if (
                                ...
                                if (
                                    ...
                                    or "error_code" in text
                                    or "TEMPORARILY_UNAVAILABLE" in text
                                ):
                                    error_found = True
        ...
        if isinstance(data, dict):
            ...
            if "metrics" in data and isinstance(data["metrics"], list):
                for metric_item in data["metrics"]:
                    if isinstance(metric_item, list) and len(metric_item) == 2:
                        ...
                        elif isinstance(metric_value, (int, float)) and not isinstance(
                            metric_value, bool
                        ):
            ...
            if "score" not in row:
                ...
                if server_error_found:
                    ...
                    for key in row:
                        if "success" in key.lower() and isinstance(
                            row[key], (int, float)
                        ):
                            row["score"] = row[key]
                            break
...
def process_model_directory(model_dir):
    ...
    for eval_file in eval_files:
        row = extract_metrics_from_eval_file(
            eval_file, provider, model_name, session_files
        )
        if row is not None:
            rows.append(row)
    ...
    if "server_error_mean" in aggregate_df.columns:
        ...
        print(
            f"Saved aggregate metrics to {csv_path} with {len(aggregate_df)} rows "
...
def main():
    ...
    if not benchmark_dir.exists() or not benchmark_dir.is_dir():
        print(
            f"Error: Benchmark directory {benchmark_dir} does not exist or is not a directory"
        )
        sys.exit(1)
    ...
    print(
        f"Successfully created aggregate_metrics.csv files for {success_count} model directories"
    )
    print("You can now run generate_leaderboard.py to create the final leaderboard.")
    print(
        "Note: The server_error_mean column indicates the average rate of server errors across evaluations."
    )
