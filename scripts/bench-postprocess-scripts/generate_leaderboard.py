...
def process_csv_files(csv_files: list) -> tuple:
    ...
    leaderboard_df = (
        union_df.groupby(["provider", "model_name"])[numeric_columns]
        .mean()
        .reset_index()
    )
...
def main():
    ...
    try:
        ...
        if not csv_files:
            print(
                f"No aggregate_metrics.csv files found in any model directory under {benchmark_dir}"
            )
            sys.exit(1)

        print(
            f"Found {len(csv_files)} aggregate_metrics.csv files in model directories"
        )
        ...
        print(
            f"Leaderboard CSV with averaged metrics saved to: {leaderboard_output_path}"
        )
        ...
        if "server_error_mean" in leaderboard_df.columns:
            ...
            if not models_with_errors.empty:
                ...
                for _, row in models_with_errors.iterrows():
                    print(
                        f"  * {row['provider']} {row['model_name']} - {row['server_error_mean'] * 100:.1f}% of evaluations had server errors"
                    )
