...
export GOOSE_USER_DATA_DIR="${GOOSE_USER_DATA_DIR:-$ROOT_DIR/.preview/e2e-apps-user-data}"
export SECURITY_PREVIEW_STATE_DIR="${SECURITY_PREVIEW_STATE_DIR:-$ROOT_DIR/.preview/e2e-apps-backend}"

rm -rf "$GOOSE_USER_DATA_DIR" "$SECURITY_PREVIEW_STATE_DIR"
mkdir -p "$GOOSE_USER_DATA_DIR"
export_preview_backend_env
trap cleanup_preview_backend EXIT

pnpm --dir ui/desktop exec playwright test tests/e2e/security-apps-smoke.spec.ts --reporter=list
