...
| `GOOSE_SERVER__SECRET_KEY` | Shared secret required in the `X-Secret-Key` header on all client requests. Also the token enforced on the ACP endpoint by `goose serve --require-token`. | Secret string | Random (auto-generated) |
