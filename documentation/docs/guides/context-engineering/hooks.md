...
| `reason` | Message for a denial, on `PreToolUseResult` when `decision` is `deny`. A bounded, non-sensitive string whose wording varies by failure class. Internal command strings, expanded plugin paths, and credentials never appear here, in `blocked_by`, or in the refusal shown to the model. |
