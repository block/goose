...
Ralph Loop requires three files. You can place them in `~/.config/goose/recipes/`:

### 1. The Bash Wrapper (`ralph-loop.sh`)
...
#   RALPH_REVIEWER_MODEL  - Model for review phase (default: goose-claude-4-5-sonnet)
...
REVIEWER_MODEL="${RALPH_REVIEWER_MODEL:-goose-claude-4-5-sonnet}"
...
### 2. Work Phase Recipe (`ralph-work.yaml`)
...
### 3. Review Phase Recipe (`ralph-review.yaml`)
