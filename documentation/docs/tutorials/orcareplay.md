...
- **Free re-runs**: replaying a recorded session costs nothing and hits no provider. Useful when a session did something surprising and you want to look again without paying for it twice.
...
goose starts again, is served the recorded responses, and reaches the same answer with the network off. `reused=8/8` means every recorded exchange was used.
