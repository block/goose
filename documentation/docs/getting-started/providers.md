...
| [Lynkr](https://github.com/Fast-Editor/Lynkr)                              | Self-hosted gateway that routes each request by complexity across local models (Ollama, llama.cpp) and cloud providers, via an OpenAI-compatible API. Reduces token usage with tool-schema stripping and JSON tool-result compression. Use through the OpenAI provider. | `OPENAI_HOST` (set to `http://localhost:8081`)                                                                                                                                       |
