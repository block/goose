...
Goose supports adding custom OpenAI-compatible providers with additional authentication headers. This is useful for:
- Corporate API gateways that require extra authentication headers
- Multi-tenant systems needing tenant identifiers
- Services requiring additional API keys or tokens
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="Goose CLI">
    ...
    4. Enter provider details:
       - Display name (e.g., "Corporate API")
       - API URL (e.g., `https://api.company.com/v1/chat/completions`)
       - API key
       - Available models (comma-separated)
    5. When prompted "Does this provider require custom headers?", select Yes
    6. Add headers as needed:
       - Enter header name (e.g., `x-origin-client-id`)
       - Enter header value (will be masked for security)
...
```json
{
  ...
  "models": [
    {"name": "gpt-4", "context_limit": 128000},
...
After adding the configuration, set the API key:
```bash
