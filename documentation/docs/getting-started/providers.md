...
| [OpenZoo](https://openzoo.fun/)                                             | Pay-per-call inference through the local `npx openzoo` proxy (OpenAI-compatible, pays each call over x402 from a local burner wallet, no account; the proxy ignores the API key). The hosted `api.openzoo.fun` answers 402 unless the caller pays x402 or presents an OpenZoo subscription key (`ozk_live_...`). Models discovered dynamically from `/v1/models`. | `OPENZOO_HOST` (optional, default `http://localhost:8402`), `OPENZOO_API_KEY` (optional, any value)                                                                                  |
