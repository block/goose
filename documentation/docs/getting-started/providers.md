...
| [Auxen](https://auxen.ai/)                                                  | Per-customer **dedicated** LLM endpoints (Llama, Qwen, Mistral, Gemma, Mixtral, Phi, Command R) with OpenAI-compatible API. Each instance is a dedicated GPU billed per-minute of runtime, not per-token. | `AUXEN_API_BASE`, `AUXEN_API_KEY` |
