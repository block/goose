...
use goose::goose_bot::{cached_installation_id, GooseBotPrefs, INSTALLATION_ID_CONFIG_KEY};
use goose_server::routes::goose_bot::routes;
...
async fn goose_bot_prefs_get_put_roundtrip() {
    ...
    let body = axum::body::to_bytes(response.into_body(), usize::MAX)
        .await
        .unwrap();
    let prefs: GooseBotPrefs = serde_json::from_slice(&body).unwrap();
    assert_eq!(prefs.custom_instructions, "route-test");
}

#[tokio::test(flavor = "multi_thread")]
async fn goose_bot_status_and_disconnect_clear_install_id() {
    let state = AppState::new(true).await.unwrap();
    let app = routes(state);
