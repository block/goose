...
/// Configuration for GooseAcpAgent (library mode - no global config)
...
impl GooseAcpAgent {
    pub async fn new(builtins: Vec<String>) -> Result<Self> {
        ...
        let goose_mode = config
            .get_goose_mode()
            .unwrap_or(goose::config::GooseMode::Approve);
    ...
    pub async fn with_config(config: GooseAcpConfig) -> Result<Self> {
        ...
        agent.set_provider(config.provider.clone()).await;
...
impl GooseAcpAgent {
    ...
    async fn on_new_session(
        ...
        let manager = self.agent.session_manager();
