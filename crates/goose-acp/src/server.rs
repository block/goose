...
fn mask_secret(secret: serde_json::Value) -> String {
    ...
    let mask = "*".repeat(chars.len().saturating_sub(show_len));
...
impl GooseAcpAgent {
    ...
    async fn on_read_config(
        ...
        let response = match config.get(&req.key, req.is_secret) {
            Ok(value) if req.is_secret => ReadConfigResponse {
                value: serde_json::Value::Null,
                masked_value: Some(mask_secret(value)),
            },
            Ok(value) => ReadConfigResponse {
                value,
                masked_value: None,
            },
    ...
    async fn on_remove_config(
        ...
        let result = if req.is_secret {
            config.delete_secret(&req.key)
        } else {
            config.delete(&req.key)
        };
