    ...
    fn load_secrets(&self) -> Result<HashMap<String, String>> {
        if self.file_path.exists() {
            let content = std::fs::read_to_string(&self.file_path)
                .map_err(|e| SecretError::StorageFailure(format!("Failed to read secrets file: {}", e)))?;
    ...
    fn save_secrets(&self, secrets: &HashMap<String, String>) -> Result<()> {
        if let Some(parent) = self.file_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| SecretError::StorageFailure(format!("Failed to create secrets directory: {}", e)))?;
        }
        let content = serde_yaml::to_string(secrets)
            .map_err(|e| SecretError::StorageFailure(format!("YAML serialize error: {}", e)))?;
        std::fs::write(&self.file_path, content)
            .map_err(|e| SecretError::StorageFailure(format!("Failed to write secrets file: {}", e)))?;
        Ok(())
    }
...
impl LegacyConfigStore {
        ...
        T: serde::de::DeserializeOwned,
    {
        ...
        let value: Value = serde_json::from_str(&secret_str)
            .unwrap_or_else(|_| Value::String(secret_str));
...
mod tests {
    ...
    fn test_file_backed_store() {
        ...
        assert!(store.set_secret("test.service", "test_key", "test_secret").is_ok());
    ...
    fn test_legacy_config_store() {
        ...
        assert!(store.set_secret("api_key", serde_json::Value::String("secret123".to_string())).is_ok());
        assert!(store.set_secret("port", serde_json::Value::Number(8080.into())).is_ok());
        assert!(store.set_secret("enabled", serde_json::Value::Bool(true)).is_ok());
