...
impl AutoVisualiserServer {
    ...
    #[tool(
        name = "render_sankey",
        description = "show a Sankey diagram from flow data. The data must contain: nodes (Array of objects with 'name' and optional 'category' properties) and links (Array of objects with 'source', 'target', and 'value' properties)"
    )]
    pub async fn render_sankey(
        ...
        let params = params.0;
        let data = Self::validate_data_param(&params.data, false)?;
    ...
    #[tool(
        name = "render_radar",
        description = "show a radar chart (spider chart) for multi-dimensional data comparison. The data must contain: labels (Array of strings representing the dimensions/axes) and datasets (Array of dataset objects with 'label' and 'data' properties)"
    )]
    ...
    #[tool(
        name = "render_donut",
        description = "show pie or donut charts for categorical data visualization. Supports single or multiple charts in a grid layout. Each chart should contain: data (Array of values or objects with 'label' and 'value'), type (Optional 'doughnut' or 'pie'), title (Optional chart title), labels (Optional array of labels if data is just numbers)"
    )]
    pub async fn render_donut(
        ...
        let params = params.0;
        let data = Self::validate_data_param(&params.data, true)?; // true because donut accepts arrays
    ...
    #[tool(
        name = "render_chord",
        description = "Show a chord diagram visualization for showing relationships and flows between entities. The data must contain: labels (Array of strings representing the entities) and matrix (2D array of numbers representing flows, matrix[i][j] = flow from i to j)"
    )]
    ...
    #[tool(
        name = "render_map",
        description = "show an interactive map visualization with location markers using Leaflet. The data must contain: markers (Array of objects with 'lat', 'lng', and optional properties), title (Optional title for the map), subtitle (Optional subtitle), center (Optional center point {lat, lng}), zoom (Optional initial zoom level), clustering (Optional boolean to enable/disable clustering), autoFit (Optional boolean to auto-fit map to markers)"
    )]
    ...
    #[tool(
        name = "show_chart",
        description = "show interactive line, scatter, or bar charts. Required: type ('line', 'scatter', or 'bar'), datasets array. Optional: labels, title, subtitle, xAxisLabel, yAxisLabel, options"
    )]
