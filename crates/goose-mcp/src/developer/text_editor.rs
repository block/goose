...
pub fn enable_acp_mode() {
    ACP_MODE.store(true, Ordering::Relaxed);
}

fn is_acp_mode() -> bool {
    ACP_MODE.load(Ordering::Relaxed)
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GooseFileDiffEnvelope {
    #[serde(rename = "__goose_file_diff__")]
    pub diff: FileDiff,
}
...
fn create_file_diff_content(path: &Path, old_text: Option<String>, new_text: &str) -> Content {
    let envelope = GooseFileDiffEnvelope {
        diff: FileDiff {
    ...
    let json_str = serde_json::to_string(&envelope).unwrap_or_else(|e| {
        tracing::error!(error = %e, "FileDiff serialization failed unexpectedly");
        String::from("{}")
    });
...
async fn apply_file_change(
    ...
    if is_acp_mode() {
        ...
        Ok((old_text, final_content.into_owned()))
    } else {
        write_file(path, &final_content).await?;
        Ok((None, final_content.into_owned()))
    }
}
...
pub async fn text_editor_write(path: &Path, file_text: &str) -> Result<Vec<Content>, ErrorData> {
    let (old_text, new_text) = apply_file_change(path, file_text).await?;
    ...
    if is_acp_mode() {
        result.push(create_file_diff_content(path, old_text, &new_text));
    }
...
pub async fn text_editor_replace(
    ...
    if let Some(ref editor) = editor_model {
        ...
        match editor.edit_code(&content, old_str, new_str).await {
            Ok(updated_content) => {
                let (old_text, new_text) = apply_file_change(path, &updated_content).await?;
    ...
    let (old_text, written_content) = apply_file_change(path, &new_content).await?;
    ...
    if is_acp_mode() {
        result.push(create_file_diff_content(path, old_text, &written_content));
    }
...
pub async fn text_editor_undo(
    ...
    let (old_text, written_content) = apply_file_change(path, &previous_content).await?;
