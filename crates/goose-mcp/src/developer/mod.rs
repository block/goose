...
impl DeveloperRouter {
    ...
    async fn bash(
        ...
        let command =
            params
                .get("command")
                .and_then(|v| v.as_str())
                .ok_or_else(|| ErrorData::new(ErrorCode::INVALID_PARAMS, "The command string is required".to_string(), None))?;
        ...
        let output_str = match output_task.await {
            Ok(result) => result.map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, e.to_string()))?,
            Err(e) => return Err(ErrorData::new(ErrorCode::INTERNAL_ERROR, e.to_string())),
        };
    ...
    async fn text_editor(&self, params: Value) -> Result<Vec<Content>, ErrorData> {
        let command = params
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                ErrorData::new(ErrorCode::INVALID_PARAMS, "Missing 'command' parameter".to_string())
            })?;
    ...
    async fn text_editor_view(
        ...
        let f = File::open(path)
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to open file: {}", e)))?;

        let file_size = f
            .metadata()
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to get file metadata: {}", e)))?
            .len();
        ...
        f.read_to_string(&mut content)
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to read file: {}", e)))?;
    ...
    async fn text_editor_write(
        ...
        std::fs::write(path, &normalized_text) // Write the potentially modified text
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to write file: {}", e)))?;
    ...
    async fn text_editor_replace(
            ...
            match editor.edit_code(&content, old_str, new_str).await {
                Ok(updated_content) => {
                    ...
                    std::fs::write(path, &normalized_content).map_err(|e| {
                        ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to write file: {}", e))
                    })?;
        ...
        if content.matches(old_str).count() == 0 {
            return Err(ErrorData::new(ErrorCode::INVALID_PARAMS, "'old_str' must appear exactly once in the file, but it does not appear in the file. Make sure the string exactly matches existing file content, including whitespace!".into(), None,
            ));
        }
    ...
    async fn text_editor_undo(&self, path: &PathBuf) -> Result<Vec<Content>, ErrorData> {
        ...
        if let Some(contents) = history.get_mut(path) {
            if let Some(previous_content) = contents.pop() {
                ...
                Err(ErrorData::new(ErrorCode::INVALID_PARAMS, "No edit history available to undo".into(), None,
                ))
            }
        } else {
            Err(ErrorData::new(ErrorCode::INVALID_PARAMS, "No edit history available to undo".into(), None,
    ...
    async fn image_processor(&self, params: Value) -> Result<Vec<Content>, ErrorData> {
        ...
        let image = xcap::image::open(&path)
            .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to open image file: {}", e)))?;
        ...
        processed_image
            ...
            .map_err(|e| {
                ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to write image buffer: {}", e))
            })?;
    ...
    async fn screen_capture(&self, params: Value) -> Result<Vec<Content>, ErrorData> {
            ...
            params.get("window_title").and_then(|v| v.as_str())
        {
            ...
            monitor.capture_image().map_err(|e| {
                ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to capture display {}: {}", display, e))
            })?
        };
    ...
    fn call_tool(
        ...
        Box::pin(async move {
            match tool_name.as_str() {
                ...
                _ => Err(ErrorData::new(ErrorCode::METHOD_NOT_FOUND, format!("Tool {} not found", tool_name))),
            }
        })
    }
