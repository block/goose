...
use self::shell::{expand_path, get_shell_config, is_absolute_path};
use indoc::indoc;
    ...
    async fn text_editor(&self, params: Value) -> Result<Vec<Content>, ErrorData> {
        ...
        match command {
            "view" => {
                ...
                text_editor::text_editor_view(&path, view_range).await
            }
            "write" => {
                let file_text = require_str_parameter(&params, "file_text")?;

                text_editor::text_editor_write(&path, file_text).await
            }
            "str_replace" | "edit_file" => {
                ...
                text_editor::text_editor_replace(
                    &path,
                    old_str,
                    new_str,
                    &self.editor_model,
            ...
            "insert" => {
                ...
                text_editor::text_editor_insert(&path, insert_line, new_str, &self.file_history)
                    .await
            }
            "undo_edit" => text_editor::text_editor_undo(&path, &self.file_history).await,
            _ => Err(ErrorData::new(
                ErrorCode::INVALID_PARAMS,
...
mod tests {
    ...
    async fn test_text_editor_view_large_file_without_range() {
        ...
        for i in 1..=text_editor::LINE_READ_LIMIT + 1 {
