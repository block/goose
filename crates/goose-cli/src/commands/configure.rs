...
/// A selectable entry in the provider picker. Distinguishes real providers from
/// the synthetic "search" UI actions so control flow is checked by the compiler
/// instead of relying on sentinel string values.
...
/// Build cliclack select items from provider metadata tuples.
