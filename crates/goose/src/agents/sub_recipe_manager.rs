...
use crate::{agents::{recipe_tools::sub_recipe_tools::{create_sub_recipe_tool, run_sub_recipe_tool, SUB_RECIPE_TOOL_NAME_PREFIX}, tool_execution::ToolCallResult}, recipe::SubRecipe};
...
impl SubRecipeManager {
    ...
    pub async fn run_sub_recipe(&self, tool_name: &str, _params: Value) -> Result<Vec<Content>, ToolError> {
        let sub_recipe_name = tool_name
            .strip_prefix(SUB_RECIPE_TOOL_NAME_PREFIX)
            .and_then(|s| s.strip_prefix("_"))
            .ok_or_else(|| ToolError::InvalidParameters(format!(
                "Invalid sub-recipe tool name format: {}", tool_name
            )))?;

        let sub_recipe = self.sub_recipes
            .get(sub_recipe_name)
            .ok_or_else(|| ToolError::InvalidParameters(format!(
                "Sub-recipe '{}' not found", sub_recipe_name
            )))?;
        
        let output = run_sub_recipe_tool(sub_recipe, _params).map_err(|e| {
