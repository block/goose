...
pub struct AutoPilot {
    ...
    switch_active: bool,
    second_opinion_last_turn: Option<usize>, // Track when second-opinion was last invoked
}
...
impl AutoPilot {
    ...
    fn should_switch_to_second_opinion(&self, conversation: &Conversation) -> bool {
        ...
        // Check if at least 5 turns have passed since last invocation
        let enough_turns_passed = if let Some(last_turn) = self.second_opinion_last_turn {
            current_turn > last_turn + 5 // More than 5 turns, not equal to
        } else {
            true // Never invoked before, so we can trigger
        };

        if !enough_turns_passed {
            return false; // Not enough turns have passed
        }
        ...
        // Check for "help" trigger word - this triggers regardless of tool usage
        if let Some(text) = last_user_message {
        ...
        // Additional trigger: If > 5 turns since last invoked AND there's a recent tool call
        // Check if the most recent assistant message contains tool calls
        let has_recent_tool_call = messages
            .iter()
            .rev()
        ...
        // Trigger if enough turns have passed AND there's a recent tool call
    ...
    pub async fn check_for_switch(
        ...
        if self.should_switch_to_second_opinion(conversation) {
            ...
            let current_turn = conversation
                ...
                .filter(|msg| msg.role == rmcp::model::Role::User)
                .count();
            self.second_opinion_last_turn = Some(current_turn);
...
mod tests {
    ...
    fn test_should_switch_to_second_opinion_with_trigger() {
        ...
        // Create a conversation with just a user message with "help"
    ...
    #[test]
    fn test_should_not_switch_to_second_opinion_without_trigger() {
        let autopilot = create_test_autopilot();

        // Without "help" trigger, should not switch
        let conversation = create_conversation_with_text("Just a normal message");
    ...
    fn test_case_insensitive_triggers() {
        ...
        // Test uppercase trigger words for oracle
        let conversation = create_conversation_with_text("I need to THINK about this");
        assert!(autopilot.should_switch_to_oracle(&conversation));

        // Test uppercase trigger words for second-opinion
        let conversation = create_conversation_with_text("I need HELP with this");
        assert!(autopilot.should_switch_to_second_opinion(&conversation));
    }

    #[test]
    fn test_should_not_switch_to_second_opinion_before_5_turns_since_last() {
        let mut autopilot = create_test_autopilot();

        // Set that second-opinion was invoked at turn 2
        autopilot.second_opinion_last_turn = Some(2);

        // Create a conversation with 7 user messages (turns)
        // Turn 7 is exactly 2 + 5, not more than 5
        let mut messages = vec![];
        for i in 0..6 {
            messages.push(Message::user().with_text(&format!("Message {}", i)));
            messages.push(Message::assistant().with_text(&format!("Response {}", i)));
        }
        messages.push(Message::user().with_text("lets do the thing with help"));

        let conversation = Conversation::new(messages).expect("Failed to create conversation");

        // Should not switch because only 5 turns have passed since turn 2 (7 - 2 = 5, not > 5)
        assert!(!autopilot.should_switch_to_second_opinion(&conversation));
    }

    #[test]
    fn test_should_switch_to_second_opinion_after_5_turns_since_last() {
        let mut autopilot = create_test_autopilot();

        // Set that second-opinion was invoked at turn 2
        autopilot.second_opinion_last_turn = Some(2);

        // Create a conversation with 8 user messages (turns)
        // Turn 8 is 2 + 6, more than 5 turns since last invocation
        let mut messages = vec![];
        for i in 0..7 {
            messages.push(Message::user().with_text(&format!("Message {}", i)));
            messages.push(Message::assistant().with_text(&format!("Response {}", i)));
        ...
        // Should switch because more than 5 turns have passed since turn 2 (8 - 2 = 6)
        assert!(autopilot.should_switch_to_second_opinion(&conversation));
    }

    #[test]
    fn test_should_switch_to_second_opinion_with_tool_call_after_5_turns() {
        use mcp_core::tool::ToolCall;
        use serde_json::json;
        ...
        // Create a conversation with 7 user messages (turns)
        // Turn 8 is > 2 + 5, so it should trigger with tool call even without "help"
        let mut messages = vec![];
        for i in 0..6 {
            messages.push(Message::user().with_text(&format!("Message {}", i)));
            messages.push(Message::assistant().with_text(&format!("Response {}", i)));
        ...
        // Add a user request and assistant with tool call
        messages.push(Message::user().with_text("Do something without help keyword"));
        messages.push(
            Message::assistant()
                .with_tool_request("tool-1", Ok(ToolCall::new("test_tool", json!({})))),
        );
        messages.push(Message::user().with_tool_response("tool-1", Ok(vec![])));
        messages.push(Message::user().with_text("Just a normal message"));

        let conversation = Conversation::new(messages).expect("Failed to create conversation");

        // Should switch because > 5 turns have passed AND there's a tool call
        assert!(autopilot.should_switch_to_second_opinion(&conversation));
    }

    #[test]
    fn test_should_not_switch_with_tool_call_before_5_turns() {
        use mcp_core::tool::ToolCall;
        use serde_json::json;
        ...
        // Create a conversation with only 6 user messages (turns)
        // Turn 6 is exactly 2 + 4, not enough
        let mut messages = vec![];
        for i in 0..4 {
            messages.push(Message::user().with_text(&format!("Message {}", i)));
            messages.push(Message::assistant().with_text(&format!("Response {}", i)));
        ...
        // Add a user request and assistant with tool call
        messages.push(Message::user().with_text("Do something"));
        messages.push(
            Message::assistant()
                .with_tool_request("tool-1", Ok(ToolCall::new("test_tool", json!({})))),
        );
        ...
        // Should NOT switch because only 4 turns have passed (6 - 2 = 4)
        assert!(!autopilot.should_switch_to_second_opinion(&conversation));
    }

    #[test]
    fn test_should_switch_with_tool_call_on_first_invocation() {
        use mcp_core::tool::ToolCall;
        use serde_json::json;

        let autopilot = create_test_autopilot();
        // No previous invocation (second_opinion_last_turn is None)

        // Create a conversation with a tool call
        let user_msg = Message::user().with_text("Do something");
        let assistant_msg = Message::assistant()
            .with_tool_request("tool-1", Ok(ToolCall::new("test_tool", json!({}))));
        let tool_response = Message::user().with_tool_response("tool-1", Ok(vec![]));
        let user_msg2 = Message::user().with_text("Just a normal message");

        let conversation =
            Conversation::new(vec![user_msg, assistant_msg, tool_response, user_msg2])
                .expect("Failed to create conversation");

        // Should switch because it's never been invoked before AND there's a tool call
        assert!(autopilot.should_switch_to_second_opinion(&conversation));
    }
}
