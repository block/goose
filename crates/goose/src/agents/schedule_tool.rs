...
impl Agent {
    ...
    pub async fn handle_schedule_management(
        ...
        let action = arguments
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErrorData::new(ErrorCode::INTERNAL_ERROR, "Missing 'action' parameter".to_string(), None))?;
        ...
        match scheduler.list_scheduled_jobs().await {
            Ok(jobs) => {
                let jobs_json = serde_json::to_string_pretty(&jobs)
                    .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Failed to serialize jobs: {}", e), None))?;
    ...
    async fn handle_create_job(
        ...
        let recipe_path = arguments
            .get("recipe_path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErrorData::new(ErrorCode::INTERNAL_ERROR, "Missing 'recipe_path' parameter".to_string(), None))?;

        let cron_expression = arguments
            .get("cron_expression")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErrorData::new(ErrorCode::INTERNAL_ERROR, "Missing 'cron_expression' parameter".to_string(), None))?;
        ...
        if execution_mode != "foreground" && execution_mode != "background" {
            return Err(ErrorData::new(
                ErrorCode::INTERNAL_ERROR,
                format!("Invalid execution_mode: {}. Must be 'foreground' or 'background'", execution_mode),
                None,
            ));
        }
            ...
            Ok(content) => {
                if recipe_path.ends_with(".json") {
                    serde_json::from_str::<Recipe>(&content)
                        .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Invalid JSON recipe: {}", e), None))?;
                } else {
                    serde_yaml::from_str::<Recipe>(&content)
                        .map_err(|e| ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Invalid YAML recipe: {}", e), None))?;
                }
            }
            Err(e) => {
                return Err(ErrorData::new(
    ...
    async fn handle_run_now(
        ...
        let job_id = arguments
            .get("job_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErrorData::new(ErrorCode::INTERNAL_ERROR, "Missing 'job_id' parameter".to_string(), None))?;
    ...
    async fn handle_session_content(
        ...
        let session_id = arguments
            .get("session_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErrorData::new(ErrorCode::INTERNAL_ERROR, "Missing 'session_id' parameter".to_string(), None))?;
