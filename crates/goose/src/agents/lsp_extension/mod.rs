...
use crate::lsp::{LspClient, LspConfig};
use anyhow::anyhow;
...
pub struct LspMcpClient {
    ...
    config: LspConfig,
    init_result: rmcp::model::InitializeResult,
}

impl LspMcpClient {
    pub async fn new(config: LspConfig) -> anyhow::Result<Self> {
...
Use these tools to analyze and navigate {} code:
- get_diagnostics: Check for errors, warnings, and other diagnostics in a file
...
- line and character: Position in the file (0-indexed)

These tools help understand code structure, find issues, and navigate codebases efficiently.",
                config.language_id, config.language_id
            )
        });

        let init_result = rmcp::model::InitializeResult {
    ...
    async fn call_hover(&self, args: Value) -> anyhow::Result<Vec<Content>> {
        let file_path = args
            .get("file_path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("Missing file_path parameter"))?;
    ...
    async fn call_goto_definition(&self, args: Value) -> anyhow::Result<Vec<Content>> {
        let file_path = args
            .get("file_path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("Missing file_path parameter"))?;
    ...
    async fn call_find_references(&self, args: Value) -> anyhow::Result<Vec<Content>> {
        let file_path = args
            .get("file_path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("Missing file_path parameter"))?;
