...
pub fn process_tool_response(response: Result<Vec<Content>, ToolError>) -> Result<Vec<Content>, ToolError> {
    match response {
        Ok(contents) => {
...
mod tests {
    use super::*;
    use mcp_core::{Content, TextContent, ImageContent, ToolError};
    use std::fs;
    use std::path::Path;
    ...
    fn test_large_text_response_redirected_to_file() {
        ...
        assert_eq!(processed.len(), 1);
        if let Content::Text(text_content) = &processed[0] {
            assert!(text_content.text.contains("The response returned from the tool call was larger"));
            assert!(text_content.text.contains("characters"));
            assert!(text_content.text.contains("stored in the file:"));
            
            // Extract the file path from the message
            if let Some(file_path) = text_content.text
