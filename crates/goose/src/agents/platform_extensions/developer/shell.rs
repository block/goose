...
async fn run_command(
    ...
    let output_truncated =
        match tokio::time::timeout(Duration::from_secs(OUTPUT_DRAIN_TIMEOUT_SECS), output_task)
            .await
        {
            Ok(Ok(Ok(()))) => false,
            Ok(Ok(Err(e))) => {
            ...
            Err(_) => {
                tracing::debug!(
                "output drain timed out after {OUTPUT_DRAIN_TIMEOUT_SECS}s (backgrounded process?)"
            );
                abort_handle.abort();
                true
            }
