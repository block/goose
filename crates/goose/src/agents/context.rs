...
use crate::context_mgmt::summarize_bp::summarize_messages_bullet_points;
use crate::context_mgmt::summarize_concise::summarize_messages_concise;
...
impl Agent {
    ...
    /// Public API to summarize the conversation using bullet points so that its token count is within the allowed context limit.
    pub async fn summarize_context_bullet_points(
        &self,
        messages: &[Message], // last message is a user msg that led to assistant message with_context_length_exceeded
    ) -> Result<(Vec<Message>, Vec<usize>), anyhow::Error> {
        let provider = self.provider().await?;
        ...
        let (mut new_messages, mut new_token_counts) =
            summarize_messages_bullet_points(provider, messages, &token_counter, target_context_limit).await?;

        // If the summarized messages only contains one message, it means no tool request and response message in the summarized messages,
        // Add an assistant message to the summarized messages to ensure the assistant's response is included in the context.
        if new_messages.len() == 1 {
            let assistant_message = Message::assistant().with_text(
                "I had run into a context length exceeded error so I summarized our conversation into bullet points.",
            );
            new_messages.push(assistant_message.clone());
            new_token_counts.push(token_counter.count_chat_tokens("", &[assistant_message], &[]));
        }
    ...
    /// Public API to summarize the conversation concisely so that its token count is within the allowed context limit.
    pub async fn summarize_context_concise(
        &self,
        messages: &[Message], // last message is a user msg that led to assistant message with_context_length_exceeded
    ) -> Result<(Vec<Message>, Vec<usize>), anyhow::Error> {
        let provider = self.provider().await?;
        ...
        let (mut new_messages, mut new_token_counts) =
            summarize_messages_concise(provider, messages, &token_counter, target_context_limit).await?;

        // If the summarized messages only contains one message, it means no tool request and response message in the summarized messages,
        // Add an assistant message to the summarized messages to ensure the assistant's response is included in the context.
        if new_messages.len() == 1 {
            let assistant_message = Message::assistant().with_text(
                "I had run into a context length exceeded error so I summarized our conversation concisely.",
            );
            new_messages.push(assistant_message.clone());
            new_token_counts.push(token_counter.count_chat_tokens("", &[assistant_message], &[]));
        }
