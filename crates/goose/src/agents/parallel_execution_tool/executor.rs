...
use crate::agents::parallel_execution_tool::lib::{Config, ExecutionResponse, ExecutionStats, Task, TaskResult};
...
pub async fn parallel_execute(tasks: Vec<Task>, config: Config) -> ExecutionResponse {
    ...
    let scaler_handle = tokio::spawn(async move {
        run_scaler(scaler_state, task_count, config.max_workers, config.timeout_seconds).await;
    });
