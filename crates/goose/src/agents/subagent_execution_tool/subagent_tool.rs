...
use crate::agents::subagent_execution_tool::task_types::{Task, TaskPayload, ExecutionMode};
...
pub fn create_subagent_tool() -> Tool {
    ...
    let schema_value = serde_json::to_value(schema).expect("Failed to serialize SubagentParams schema");
    let input_schema = schema_value.as_object().expect("Schema should be an object").clone();
    ...
    Tool::new(
        ...
        Defaults to 'return_last_only=true' for concise results.".to_string(),
        input_schema,
    ...
    match prompt_template::render_global_file(template_path, &context) {
        ...
        Err(e) => {
            ...
            if !matches!(subagent_type, SubagentType::Default) {
                 tracing::warn!("Failed to load subagent persona '{}': {}. Falling back to default.", template_path, e);
...
pub async fn run_subagent_tool(
    ...
    let params: SubagentParams = match serde_json::from_value(params) {
        Ok(p) => p,
        Err(e) => return ToolCallResult::from(Err(ErrorData {
    ...
    let provider_instance = match provider_guard.as_ref() {
        Some(p) => p.clone(),
        None => return ToolCallResult::from(Err(ErrorData {
            code: ErrorCode::INTERNAL_ERROR,
