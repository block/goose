...
mod tests {
    ...
    #[async_trait::async_trait]
    impl Provider for PromptCapturingNamingProvider {
        fn get_name(&self) -> &str {
            "prompt-capturing-naming-test"
        }
        ...
        async fn complete(
            ...
            let texts = messages
                ...
                .collect::<Vec<_>>();
            self.captured.lock().unwrap().extend(texts);
            Ok((
                Message::assistant().with_text(GENERATED_SESSION_NAME),
                ProviderUsage::new("test".to_string(), Default::default()),
            ))
    ...
    async fn test_maybe_update_name_feeds_working_dir_hints_into_prompt() {
        ...
        let provider = Arc::new(PromptCapturingNamingProvider {
            captured: std::sync::Mutex::new(Vec::new()),
        });
        sm.maybe_update_name(&session.id, provider.clone())
            .await
            .unwrap();

        let captured = provider.captured.lock().unwrap().join("\n");
        assert!(
