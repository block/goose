...
/// Default content for templates whose canonical copy lives outside this
/// crate (the compaction templates moved to `goose-providers`), falling back
/// to the embedded prompts directory.
fn default_template_content(name: &str) -> Option<String> {
    match name {
        "compaction.md" => Some(goose_providers::context_mgmt::COMPACTION_PROMPT_TEMPLATE.into()),
        "compaction_summary.md" => {
            Some(goose_providers::context_mgmt::COMPACTION_SUMMARY_TEMPLATE.into())
        }
        _ => CORE_PROMPTS_DIR
            .get_file(name)
...
/// Returns the user's customized content for a registered template, if any.
pub fn user_template_override(name: &str) -> Option<String> {
    if !is_registered(name) {
        return None;
    }
    std::fs::read_to_string(user_prompts_dir().join(name)).ok()
}
...
pub fn render_template<T: Serialize>(name: &str, context: &T) -> Result<String, MiniJinjaError> {
    ...
    let template_str = if user_path.exists() {
        ...
        default_template_content(name).ok_or_else(|| {
            MiniJinjaError::new(
                minijinja::ErrorKind::TemplateNotFound,
...
pub fn get_template(name: &str) -> Option<Template> {
    ...
    let default_content = default_template_content(name)?;
