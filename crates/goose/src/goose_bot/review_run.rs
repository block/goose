...
pub async fn run_review(req: GooseBotReviewRequest) -> Result<Vec<Finding>> {
    ...
    if let Err(e) = &result {
        if let Some(crid) = req.check_run_id {
            ...
            {
                tracing::warn!("[goose_bot] complete failed review check run: {complete_err}");
            }
        }
    }
    result
...
async fn run_review_inner(req: &GooseBotReviewRequest) -> Result<Vec<Finding>> {
