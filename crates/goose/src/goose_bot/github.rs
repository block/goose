...
use super::types::GooseBotReviewRequest;
use super::{GooseBotPrefs, ReviewOutputStyle};

const USER_AGENT: &str = concat!("goose-bot/", env!("CARGO_PKG_VERSION"));
...
pub async fn fetch_pr_base_sha(req: &GooseBotReviewRequest) -> Result<String> {
    #[derive(Deserialize)]
    struct PrResponse {
        base: BaseRef,
    }
...
pub async fn post_review(
    ...
    tracing::warn!(
        "[goose_bot] inline review rejected ({status}): {detail_short} - falling back to summary-only"
    );
...
pub async fn complete_check_run(
    ...
    client()
        ...
        .json(&serde_json::json!({
            ...
            "output": {
                "title": title,
                "summary": "Goose Bot review complete.",
            },
        }))
        .send()
        .await?
...
pub async fn replace_comment_reaction(
    ...
    for r in existing {
        let is_ours = r
            .user
            .as_ref()
            .map(|u| u.login.starts_with("aaif-goose-bot"))
            .unwrap_or(false);
...
pub async fn configure_bot_git_identity(workdir: &Path) -> Result<()> {
    git_run(
        ...
        &[
            "config",
            "user.email",
            "aaif-goose-bot[bot]@users.noreply.github.com",
        ],
    )
    .await?;
    git_run(workdir, &["config", "user.name", "aaif-goose-bot[bot]"]).await
}
