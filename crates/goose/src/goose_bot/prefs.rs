...
impl Default for GooseBotPrefs {
    fn default() -> Self {
        Self {
...
impl Default for RoutingPrefs {
    fn default() -> Self {
        GooseBotPrefs::default().routing_subset()
    }
}
...
mod tests {
    ...
    fn defaults_roundtrip() {
        let p = GooseBotPrefs::default();
        let json = serde_json::to_string(&p).unwrap();
        let p2: GooseBotPrefs = serde_json::from_str(&json).unwrap();
        assert_eq!(p, p2);
    }

    #[test]
    fn missing_fields_get_defaults() {
        let p: GooseBotPrefs = serde_json::from_str("{}").unwrap();
        assert_eq!(p, GooseBotPrefs::default());
    }

    #[test]
    fn partial_payload_merges_defaults() {
        let p: GooseBotPrefs = serde_json::from_str(
    ...
    fn validate_accepts_default() {
        GooseBotPrefs::default().validate().unwrap();
    }
