...
pub async fn run_comment_reply(req: GooseBotCommentRequest) -> Result<bool> {
...
fn can_push_to_pr_head(req: &GooseBotCommentRequest) -> bool {
...
async fn fetch_comment_context(req: &GooseBotCommentRequest) -> Result<CommentContext> {
    #[derive(Deserialize)]
...
async fn fetch_pr_base_ref(req: &GooseBotCommentRequest) -> Result<String> {
    #[derive(Deserialize)]
    struct PrResponse {
        base: BaseRef,
    }
...
{custom}\n\
The commenter's message (with @aaif-goose-bot stripped):\n\
---\n\
{body}\n\
---\n\
\n\
...
async fn post_pr_comment(req: &GooseBotCommentRequest, reply: &str) -> Result<()> {
    let url = format!(
...
async fn create_branch_and_open_pr_if_changed(
    ...
    let suffix = SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let branch = format!("goose-bot/issue-{}-{suffix}", req.pr_number);
    ...
    let body = format!(
        "Closes #{issue}.\n\nRequested by @{user} via Goose Bot.",
        issue = req.pr_number,
        user = req.commenter,
    );
...
mod tests {
    ...
    fn pr_head_push_requires_same_repo_when_head_repo_is_known() {
        let mut req: GooseBotCommentRequest = serde_json::from_str(
            r#"{
                "github_token": "t",
                "repo": "owner/repo",
                "pr_number": 1,
