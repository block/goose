...
impl GenericHttpProvider {
    ...
    fn build_auth_header(&self) -> Result<Option<(String, String)>> {
        match &self.config.auth {
            ...
            AuthConfig::Basic {
                ...
                Ok(Some(("Authorization".to_string(), format!("Basic {}", credentials))))
            }
            AuthConfig::Query { .. } => {
    ...
    fn parse_response(&self, response: &Value) -> Result<(String, Vec<ParsedToolCall>), ProviderError> {
                ...
                if !error.is_null() {
                    let error_msg = match error {
                        ...
                        Value::Array(arr) => {
                            ...
                            arr.iter()
                                .filter_map(|e| {
                                    let error_type = e.get("type").and_then(|v| v.as_str()).unwrap_or("error");
                                    let msg = e.get("msg").and_then(|v| v.as_str()).unwrap_or("");
                                    Some(format!("[{}] {}", error_type, msg))
                                })
                                .collect::<Vec<_>>()
                                .join("; ")
                        }
    ...
    fn build_request(&self, body: &Value) -> Result<reqwest::RequestBuilder, ProviderError> {
        let url = self.build_url().map_err(|e| {
            ProviderError::RequestFailed(format!("Failed to build URL: {}", e))
        })?;

        let headers = self.build_headers().map_err(|e| {
            ProviderError::RequestFailed(format!("Failed to build headers: {}", e))
        })?;
...
impl Provider for GenericHttpProvider {
    ...
    async fn complete_with_model(
        ...
        let body = self.build_request_body(system, messages, tools, false).map_err(|e| {
    ...
    fn supports_streaming(&self) -> bool {
        self.config.streaming.as_ref().map(|s| s.enabled).unwrap_or(false)
    }
    ...
    async fn stream(
        ...
        let body = self.build_request_body(system, messages, tools, true).map_err(|e| {
            ProviderError::RequestFailed(format!("Failed to build request body: {}", e))
        })?;
        ...
        Ok(Box::pin(try_stream! {
            ...
            if let Some(tool_config) = &tool_injection_config {
                ...
                if !tool_calls.is_empty() {
                    ...
                    yield (Some(final_message), Some(ProviderUsage::new(model_name.clone(), usage)));
                }
            }

            // Final yield with usage if we have it
            if final_usage.is_some() {
                yield (None, Some(ProviderUsage::new(model_name, final_usage.unwrap_or_default())));
            }
        }))
    }
}
