...
pub fn generate_tool_injection(tools: &[Tool], config: &ToolInjectionConfig) -> String {
    ...
    match config.format {
        ...
        ToolInjectionFormat::Xml => {
            ...
            injection
                .push_str(r#"<arguments>{"param1": "value1"}</arguments>"#);
            injection.push_str(&format!("\n</{}>\n\n", config.block_name));
        }
        ToolInjectionFormat::Json => {
            injection.push_str("To use a tool, respond with a JSON object in this EXACT format:\n\n");
            injection.push_str(&format!(
...
pub fn parse_tool_calls(content: &str, config: &ToolInjectionConfig) -> (String, Vec<ParsedToolCall>) {
    if !config.enabled {
...
pub fn build_variables(
    ...
    // Build full prompt
    let prompt = format_messages(system, messages, prompt_format, message_format, role_mappings);
