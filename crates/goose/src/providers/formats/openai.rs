        ...
        if let Some(tool_calls_array) = tool_calls.as_array() {
            for tool_call in tool_calls_array {
                ...
                if !is_valid_function_name(&function_name) {
                    let error = ErrorData::new(ErrorCode::METHOD_NOT_FOUND, format!(
        ...
        if let MessageContent::ToolRequest(request) = &message.content[0] {
            match &request.tool_call {
                Err(error) => {
                    ...
                    assert!(error.message.starts_with("The provided function name"));
                }
        ...
        if let MessageContent::ToolRequest(request) = &message.content[0] {
            match &request.tool_call {
                Err(error) => {
                    ...
                    assert!(error.message.starts_with("Could not interpret tool use parameters"));
                }
