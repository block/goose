...
/// Required for native-tls since `reqwest::Identity::from_pkcs8_pem` only accepts PKCS#8.
#[cfg(feature = "native-tls")]
fn convert_key_to_pkcs8_pem(key_pem_str: &str) -> Result<String> {
    use pkcs8::der::{asn1::AnyRef, asn1::ObjectIdentifier, Decode, Encode, Tag};
    ...
    match parsed.tag() {
        ...
        "EC PRIVATE KEY" => {
            ...
            let curve_oid_der = curve_oid
                .to_der()
                .map_err(|e| anyhow::anyhow!("Failed to encode curve OID: {}", e))?;
            let params = AnyRef::new(Tag::ObjectIdentifier, &curve_oid_der)
                .map_err(|e| anyhow::anyhow!("Failed to create params: {}", e))?;
            let algorithm = pkcs8::AlgorithmIdentifierRef {
                oid: ec_oid,
...
-----END EC PRIVATE KEY-----";
    ...
    fn test_pkcs1_rsa_key_conversion() {
        ...
        // Verify it can be parsed as PKCS#8
