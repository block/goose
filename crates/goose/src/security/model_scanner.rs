...
impl ModelScanResult {
    ...
    /// Add metadata to an existing result (builder pattern)
    pub fn with_metadata_builder(mut self, metadata: serde_json::Value) -> Self {
...
impl GenericModelScanner {
    ...
    pub async fn scan_for_prompt_injection<T: ModelScanner>(
        ...
        tracing::debug!(
            "🔒 Starting model scan with {} (version: {}) for text length: {}",
            model_name,
            model_version,
            text.len()
        );
        ...
        if result.confidence < 0.0 || result.confidence > 1.0 {
            tracing::warn!(
                "🔒 Model {} returned invalid confidence: {:.3}",
                model_name,
                result.confidence
            );
        }
    ...
    pub async fn batch_scan_for_prompt_injection<T: ModelScanner>(
        ...
        tracing::debug!(
            "🔒 Starting batch model scan with {} for {} texts",
            model_name,
            texts.len()
        );
        ...
        tracing::debug!(
            "🔒 Batch scan completed for {} texts with {}",
            texts.len(),
            model_name
        );
        Ok(results)
...
impl ModelScanResult {
    ...
    pub fn from_binary_logits(safe_logit: f64, injection_logit: f64) -> Self {
        tracing::debug!(
            "Converting binary logits: safe={:.3}, injection={:.3}",
            safe_logit,
            injection_logit
        );
        ...
        tracing::debug!(
            "Softmax probabilities: safe={:.3}, injection={:.3}",
            safe_prob,
            injection_prob
        );
        ...
        tracing::debug!(
            "Final binary classification: is_injection={}, confidence={:.3}",
            is_injection,
            confidence
        );
    ...
    pub fn from_raw_scores_with_validation(scores: Vec<f64>, expected_length: usize) -> Self {
        if scores.len() != expected_length {
            tracing::warn!(
                "Expected {} scores from model, got {}",
                expected_length,
                scores.len()
            );
