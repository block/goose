...
// Constants for the bullet point summarization prompt and a follow-up user message.
const SUMMARY_PROMPT: &str = "You are an expert summarizer. Extract the key bullet points or main takeaways from the following text.";

/// Summarize the combined messages from the accumulated summary and the current chunk.
///
/// This method builds the bullet point summarization request, sends it to the provider, and returns the summarized response.
async fn summarize_combined_messages(
    ...
    // Format the batch as a bullet point summarization request.
    let request_text = format!(
        "Extract the key bullet points from this text:\n\n---\n{:?}\n---",
        combined_messages
    );
...
/// Summarizes messages into a bullet point format.
///
/// This function breaks down large conversations into manageable chunks,
/// summarizes each chunk into bullet points, and combines them into a final
/// bullet-point summary. It preserves important context like tool responses.
///
/// The bullet point approach focuses on extracting the key points or main takeaways
/// from the conversation, providing a more structured and easily scannable summary.
///
/// Summarization steps:
...
///    b. Summarize the combined text into bullet points.
/// 3. Generate a final bullet point summary.
pub async fn summarize_messages_bullet_points(
    provider: Arc<dyn Provider>,
    messages: &[Message],
...
mod tests {
    ...
    #[tokio::test]
    async fn test_summarize_messages_bullet_points_single_chunk() {
        ...
        let result = summarize_messages_bullet_points(
            Arc::clone(&provider),
            &messages,
            &token_counter,
            context_limit,
    ...
    #[tokio::test]
    async fn test_summarize_messages_bullet_points_multiple_chunks() {
        let provider = create_mock_provider();
        let token_counter = TokenCounter::new(GPT_4O_TOKENIZER);
    ...
    #[tokio::test]
    async fn test_summarize_messages_bullet_points_empty_input() {
        let provider = create_mock_provider();
        let token_counter = TokenCounter::new(GPT_4O_TOKENIZER);
