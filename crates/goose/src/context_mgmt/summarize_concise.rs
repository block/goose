...
// Constants for the concise summarization prompt and a follow-up user message.
const SUMMARY_PROMPT: &str = "You are an expert summarizer. Condense the following text, focusing on the main points and key information, while maintaining clarity and coherence.";

/// Summarize the combined messages from the accumulated summary and the current chunk.
///
/// This method builds the concise summarization request, sends it to the provider, and returns the summarized response.
async fn summarize_combined_messages(
    ...
    // Format the batch as a concise summarization request.
...
/// Summarizes messages concisely, focusing on the main points and key information.
///
/// This function breaks down large conversations into manageable chunks,
/// summarizes each chunk concisely, and combines them into a final
/// cohesive summary. It preserves important context like tool responses.
///
/// The concise approach focuses on condensing information while maintaining
/// clarity and coherence, providing a comprehensive yet shorter version
...
///    b. Summarize the combined text concisely.
/// 3. Generate a final concise summary.
pub async fn summarize_messages_concise(
    provider: Arc<dyn Provider>,
    messages: &[Message],
...
mod tests {
            ...
            Ok((
                Message {
                    ...
                    content: vec![MessageContent::Text(TextContent {
                        text: "This is a concise summary of the conversation.".to_string(),
                        annotations: None,
                    })],
                },
    ...
    #[tokio::test]
    async fn test_summarize_messages_concise_single_chunk() {
        ...
        let result = summarize_messages_concise(
            Arc::clone(&provider),
            &messages,
            &token_counter,
            context_limit,
    ...
    #[tokio::test]
    async fn test_summarize_messages_concise_multiple_chunks() {
        let provider = create_mock_provider();
        let token_counter = TokenCounter::new(GPT_4O_TOKENIZER);
    ...
    #[tokio::test]
    async fn test_summarize_messages_concise_empty_input() {
        let provider = create_mock_provider();
        let token_counter = TokenCounter::new(GPT_4O_TOKENIZER);
