...
pub async fn compact_messages(
    ...
    let (messages_to_compact, preserved_user_message) = if let Some(last_message) = messages.last()
    {
        if matches!(last_message.role, rmcp::model::Role::User) && has_text_only(last_message) {
            ...
            // Find the most recent user message with text content only and copy it (don't remove from history)
...
mod tests {
    ...
    use rmcp::model::{CallToolRequestParam, Content, Role};
    use rmcp::object;
    use std::sync::Arc;
    ...
    #[tokio::test]
    async fn test_compact_messages_preserves_text_user_message_not_tool_calls() {
        // Create a mock provider
        let mock_provider = Arc::new(MockProvider {
            response: "This is a summary of the conversation.".to_string(),
        });

        // Create an agent and set the provider
        let agent = Agent::new();
        ...
        // 3. User message with only tool request (no text)
        // 4. Tool response
        // 5. Assistant message
        let messages = vec![
            Message::user().with_text("What is the weather?"),
            Message::assistant().with_text("Let me check the weather for you."),
        ...
        // Find the last message (should be the preserved user message)
        let last_message = compacted_messages.last().unwrap();

        // Verify the last message is a user message with text content
        assert_eq!(last_message.role, Role::User);

        // Verify it has text content
        let has_text = last_message
            .content
            .iter()
            .any(|c| matches!(c, MessageContent::Text(_)));
        assert!(has_text, "Last message should have text content");

        // Verify it's the original user text message, not a tool call message
        if let MessageContent::Text(text) = &last_message.content[0] {
            assert_eq!(
                text.text, "What is the weather?",
                "Should preserve the original user text message, not tool calls"
            );
        } else {
            panic!("Expected text content in preserved message");
        }
    ...
    #[tokio::test]
    async fn test_compact_messages_no_text_user_message_preserves_none() {
        // Create a mock provider
        let mock_provider = Arc::new(MockProvider {
        ...
        // Create a conversation ending with only tool calls (no text user message)
        let messages = vec![
            Message::assistant().with_text("I'll help you with that."),
            Message::user().with_tool_request(
                "tool_1",
                Ok(CallToolRequestParam {
                    name: "get_data".into(),
            ...
            Message::assistant().with_text("Got the data."),
        ];
        ...
        // The last message should be the assistant instruction message, not a preserved user message
        // (since there's no user text message to preserve)
        let last_message = compacted_messages.last().unwrap();
        assert_eq!(last_message.role, Role::Assistant);

        // Verify no user-VISIBLE message with text was preserved
        // (the summary message is user role but agent-only, so shouldn't be visible to user)
        let has_preserved_visible_user_message = compacted_messages.iter().rev().any(|msg| {
            msg.role == Role::User
                && msg.is_user_visible()
                && msg
                    .content
        ...
        assert!(
            !has_preserved_visible_user_message,
            "Should not preserve user-visible message without text content"
        );
    }

    #[tokio::test]
    async fn test_compact_messages_mixed_text_and_tool_not_preserved() {
        // Create a mock provider
        let mock_provider = Arc::new(MockProvider {
            response: "This is a summary.".to_string(),
        });
        ...
        // Create a conversation with:
        // 1. User message with text only
        // 2. Assistant response
        // 3. User message with BOTH text AND tool request (should NOT be preserved)
        let messages = vec![
            Message::user().with_text("What is the weather?"),
            Message::assistant().with_text("Let me check the weather for you."),
            Message::user()
                .with_text("Here's some text")
                .with_tool_request(
                    "tool_1",
                    Ok(CallToolRequestParam {
                        name: "get_weather".into(),
        ...
        // Find the last user-visible message
        let last_user_visible_message = compacted_messages
            .iter()
            .rev()
            .find(|msg| msg.role == Role::User && msg.is_user_visible());

        // Verify that if there is a preserved user message, it's the text-only one
        if let Some(last_message) = last_user_visible_message {
            // Verify it doesn't have tool content
            ...
            // Verify it's the first message (text-only), not the last one (text + tool)
            if let MessageContent::Text(text) = &last_message.content[0] {
                assert_eq!(
                    text.text, "What is the weather?",
                    "Should preserve the text-only message, not the mixed text+tool message"
                );
            }
        }
    }
