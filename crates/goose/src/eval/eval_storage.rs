...
// Routing eval integration removed — eval execution will be added
// when multi-agent routing lands upstream. Dataset CRUD and analytics
...
/// Generic eval case result — compatible with any eval runner.
/// Replaces the routing-specific RoutingEvalResult for upstream compatibility.
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
#[schema(as = EvalCaseResult)]
#[serde(rename_all = "camelCase")]
...
impl<'a> EvalStorage<'a> {
    ...
    pub async fn ensure_tables(&self) -> Result<()> {
        sqlx::query(
            r#"CREATE TABLE IF NOT EXISTS eval_datasets (
                id TEXT PRIMARY KEY,
                name TEXT NOT NULL,
        ...
        sqlx::query(
            r#"CREATE TABLE IF NOT EXISTS eval_test_cases (
                id TEXT PRIMARY KEY,
    ...
    // ── Store eval run ────────────────────────────────────────────
    // Note: run_eval() with OrchestratorAgent integration will be added
    // when multi-agent routing lands upstream. For now, results can be
    // stored via store_eval_run() from external eval runners.

    /// Generic eval result for a single test case.
    /// Compatible with any eval runner (routing, tool accuracy, etc.).
    pub async fn store_eval_run(
        ...
        for r in results {
            let entry = per_agent.entry(r.expected_agent.clone()).or_insert((0, 0));
            entry.0 += 1;
            if r.fully_correct {
                entry.1 += 1;
            }
        ...
        for r in results {
            let entry = per_mode.entry(r.expected_mode.clone()).or_insert((0, 0));
            entry.0 += 1;
            if r.mode_correct {
                entry.1 += 1;
            }
        }
        let mode_correct_count = results.iter().filter(|r| r.mode_correct).count();
        let mode_accuracy = if total > 0 {
            mode_correct_count as f64 / total as f64
        } else {
            0.0
        };
        ...
        let status = if overall_accuracy >= 0.95 {
            "pass"
        } else if overall_accuracy >= 0.85 {
            "degraded"
        } else {
            "fail"
        };

        // Build stored representations
        ...
        let stored_metrics = StoredMetrics {
            ...
            per_mode: per_mode
                ...
                .collect(),
            confusion_matrix: Vec::new(), // Will be computed from stored results on read
        };
