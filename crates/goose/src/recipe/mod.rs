...
isGlobal: true"#;
    ...
    #[test]
    fn test_to_yaml_handles_special_characters() {
        let recipe = Recipe::builder()
            .title("Test Recipe")
            .description("A test recipe")
            .instructions("Check the following: 1. First item with {braces} and #hash")
            .build()
            .unwrap();

        let yaml_content = recipe.to_yaml().unwrap();

        let parsed_recipe = Recipe::from_content(&yaml_content).unwrap();

        assert_eq!(parsed_recipe.title, "Test Recipe");
        assert_eq!(parsed_recipe.description, "A test recipe");
        assert_eq!(
            parsed_recipe.instructions,
            Some("Check the following: 1. First item with {braces} and #hash".to_string())
        );
    }

    #[test]
    fn test_to_yaml_handles_multiline_with_special_chars() {
        let recipe = Recipe::builder()
            .title("Test Recipe")
            .description("A test recipe")
            .instructions(
                "Line 1: Check this\nLine 2: More details {json}\nLine 3: #comment-like text",
            )
            .build()
            .unwrap();
    ...
    #[test]
    fn test_to_yaml_handles_colons_in_description() {
        let recipe = Recipe::builder()
            .title("Test Recipe")
            .description("A recipe with: colons and --- dashes")
        ...
        assert_eq!(
            ...
            "A recipe with: colons and --- dashes"
        );
        assert_eq!(
            parsed_recipe.instructions,
