...
/// What the loopback listener hands back to the flow: the one-time code plus
/// the `state` the server echoed, so the caller can prove the redirect belongs
/// to the request it started (RFC 6749 §10.12).
#[derive(Debug)]
...
/// Run the callback server on 127.0.0.1:53682
pub async fn run_callback_server(
    code_tx: oneshot::Sender<CallbackResult>,
    shutdown_rx: oneshot::Receiver<()>,
) -> Result<()> {
    let app = Router::new().route("/", get(handle_callback));
    let addr = SocketAddr::from(([127, 0, 0, 1], 53682));
...
async fn handle_callback(
    ...
    state: axum::extract::State<
        std::sync::Arc<tokio::sync::Mutex<Option<oneshot::Sender<CallbackResult>>>>,
    >,
) -> impl IntoResponse {
