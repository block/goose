...
impl Default for Config {
    fn default() -> Self {
        ...
        let config_storage = ConfigStorage::File { config_path: config_path };
...
impl Config {
    ...
    /// Set up global in-memory configuration without using environment variables
    ///
    /// This initializes the global configuration to use in-memory storage
    ///
    /// Returns true if the global config was successfully initialized, false if it was already initialized.
    pub fn set_global_in_memory() -> bool {
        // Enable in-memory storage at the global level
        set_in_memory(true);

        // Only initialize if not already initialized
        if GLOBAL_CONFIG.get().is_none() {
            let _ = GLOBAL_CONFIG.set(Config::new_in_memory());
            true
        } else {
            false
        }
        ...
        match &self.config_storage {
            ...
            ConfigStorage::Memory { values } => {
                ...
                match values.lock() {
                    ...
                    Err(_) => Err(ConfigError::LockError("Failed to acquire mutex lock".to_string()))
                }
            }
        }
    }
        ...
        match &self.secrets {
            ...
            SecretStorage::Memory {
                ...
                match secret_values.lock() {
                    ...
                    Err(e) => return Err(ConfigError::LockError(format!("Failed to acquire mutex lock: {}", e))),
                }
            }
        };
        Ok(())
