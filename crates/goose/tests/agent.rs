...
"#;
        ...
        struct StopHookTestEnv {
            ...
            hook_log: PathBuf,
            prev_cwd: PathBuf,
            prev_goose_path_root: Option<OsString>,
            prev_stop_hook_block_cap: Option<OsString>,
        }

        impl StopHookTestEnv {
            fn new(stop_hook_block_cap: u32, script: &str) -> Result<Self> {
                let temp_dir = tempfile::tempdir()?;
                let project_dir = temp_dir.path().join("project");
                let plugin_dir = project_dir.join(".agents/plugins/stop-blocker");
...
"#,
                ...
                std::fs::create_dir_all(temp_dir.path().join("goose-root"))?;
                std::fs::create_dir_all(temp_dir.path().join("data"))?;

                let prev_cwd = std::env::current_dir()?;
                let prev_goose_path_root = std::env::var_os("GOOSE_PATH_ROOT");
                let prev_stop_hook_block_cap = std::env::var_os("GOOSE_STOP_HOOK_BLOCK_CAP");

                unsafe {
                    std::env::set_var("GOOSE_PATH_ROOT", temp_dir.path().join("goose-root"));
                    std::env::set_var("GOOSE_STOP_HOOK_BLOCK_CAP", stop_hook_block_cap.to_string());
                }
                ...
                Ok(Self {
                    ...
                    prev_cwd,
                    prev_goose_path_root,
                    prev_stop_hook_block_cap,
                })
            }
        ...
        impl Drop for StopHookTestEnv {
            fn drop(&mut self) {
                let _ = std::env::set_current_dir(&self.prev_cwd);
                restore_env_var("GOOSE_PATH_ROOT", self.prev_goose_path_root.as_ref());
                restore_env_var(
                    "GOOSE_STOP_HOOK_BLOCK_CAP",
                    self.prev_stop_hook_block_cap.as_ref(),
                );
            }
        }
        ...
        async fn run_turn(agent: &Agent, session_id: &str, text: &str) -> Result<Vec<Message>> {
            let session_config = SessionConfig {
        ...
        #[tokio::test]
        #[serial]
        async fn stop_hook_block_cap_allows_configured_consecutive_blocks_then_overrides(
        ) -> Result<()> {
            let env = StopHookTestEnv::new(2, ALWAYS_BLOCK_SCRIPT)?;
            let (agent, session_id, provider) = create_agent_and_session(&env).await?;

            let messages = run_turn(&agent, &session_id, "hello").await?;
        ...
        async fn stop_hook_block_cap_counts_only_consecutive_blocks() -> Result<()> {
            let env = StopHookTestEnv::new(1, ALTERNATE_BLOCK_ALLOW_SCRIPT)?;
            let (agent, session_id, provider) = create_agent_and_session(&env).await?;

            let first_turn = run_turn(&agent, &session_id, "first").await?;
            let second_turn = run_turn(&agent, &session_id, "second").await?;
