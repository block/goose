...
mod tests {
    ...
    mod session_extension_load_tests {
        use super::*;
        use async_trait::async_trait;
        ...
        #[tokio::test]
        async fn skips_stdio_and_streamable_http_when_provider_manages_own_context() {
            let temp_dir = TempDir::new().unwrap();
            let session_manager = Arc::new(SessionManager::new(temp_dir.path().to_path_buf()));
            let permission_manager =
                Arc::new(PermissionManager::new(temp_dir.path().to_path_buf()));
            ...
            let session = session_manager
                .create_session(
                    std::env::current_dir().unwrap(),
                    "acp-extension-skip-test".to_string(),
                    SessionType::Hidden,
                    GooseMode::default(),
                )
                .await
            ...
            agent
                .update_provider(
                    Arc::new(ManagesOwnContextProvider),
            ...
            let extensions = vec![
                ...
                ExtensionConfig::StreamableHttp {
                    name: "external-http".to_string(),
                    description: String::new(),
                    uri: "http://nonexistent.local/mcp".to_string(),
                    envs: Envs::default(),
                    env_keys: vec![],
            ...
            assert!(
                results.is_empty(),
                "external extensions must be skipped when provider manages own context, got: {results:?}"
            );
            assert!(
                !agent
                    .extension_manager
                    .is_extension_enabled("external-stdio")
                    .await,
                "Stdio extension must not be loaded when provider manages own context"
            );
            assert!(
                !agent
                    .extension_manager
                    .is_extension_enabled("external-http")
                    .await,
                "StreamableHttp extension must not be loaded when provider manages own context"
            );
        }
    }
}
