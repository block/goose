    ...
    async fn complete_with_model(
        ...
        *count += 1;
        ...
        let response = if *count < 3 {
            ...
            Message::assistant().with_text(format!("Processing step {}...", count))
        } else {
...
async fn test_agent_loop_compaction_with_different_thresholds() {
    ...
    config
        .set_param("GOOSE_AUTO_COMPACT_THRESHOLD", serde_json::Value::from(0.8))
        .unwrap();

    // Set agent threshold to 60% (more aggressive)
    config
        .set_param(
            "GOOSE_AGENT_COMPACT_THRESHOLD",
            serde_json::Value::from(0.6),
        )
        .unwrap();
...
async fn test_compaction_threshold_scenarios() {
    ...
    let check_80 =
        auto_compact::check_compaction_needed(&agent, &messages, Some(0.8), Some(&metadata))
            .await
            .unwrap();
    ...
    let check_60 =
        auto_compact::check_compaction_needed(&agent, &messages, Some(0.6), Some(&metadata))
            .await
            .unwrap();
    ...
    let check_50 =
        auto_compact::check_compaction_needed(&agent, &messages, Some(0.5), Some(&metadata))
            .await
            .unwrap();
    ...
    let check_disabled =
        auto_compact::check_compaction_needed(&agent, &messages, Some(0.0), Some(&metadata))
            .await
            .unwrap();
    ...
    let check_disabled_100 =
        auto_compact::check_compaction_needed(&agent, &messages, Some(1.0), Some(&metadata))
            .await
            .unwrap();
