...
mod tetrate_streaming_tests {
    ...
    async fn test_tetrate_streaming_basic() -> Result<()> {
        ...
        let messages = vec![
            Message::user().with_text("Count from 1 to 5, one number at a time.")
        ];
        
        let mut stream = provider
            .stream("You are a helpful assistant that counts numbers.", &messages, &[])
            .await?;
        ...
        assert!(full_content.contains('1'), "Response should contain number 1");
        assert!(full_content.contains('5'), "Response should contain number 5");
    ...
    async fn test_tetrate_streaming_with_tools() -> Result<()> {
        ...
        let mut stream = provider
            .stream("You are a helpful assistant with access to weather information.", &messages, &[weather_tool])
            .await?;
    ...
    async fn test_tetrate_streaming_empty_response() -> Result<()> {
        ...
        assert!(chunk_count > 0, "Should receive at least one chunk even with empty input");
    ...
    async fn test_tetrate_streaming_long_response() -> Result<()> {
        ...
        let messages = vec![
            Message::user().with_text("Write a detailed 3-paragraph essay about the importance of streaming in modern APIs.")
        ];
        
        let mut stream = provider
            .stream("You are a helpful assistant that writes detailed essays.", &messages, &[])
            .await?;
        ...
        while let Some(result) = stream.next().await {
            ...
            if let Some(usage_info) = usage {
                ...
                assert!(usage_info.usage.output_tokens.unwrap_or(0) > 0, "Should have output tokens");
            }
        }
        
        println!("Received {} chunks with total content length: {}", chunk_count, total_content_length);
        
        // For a detailed essay, we expect multiple chunks and substantial content
        assert!(chunk_count > 5, "Long response should be streamed in multiple chunks");
        assert!(total_content_length > 100, "Essay should have substantial content");
    ...
    async fn test_tetrate_streaming_concurrent_streams() -> Result<()> {
        ...
        assert!(content1.contains("Stream 1") || content1.contains("1"), "First stream should mention Stream 1");
        assert!(content2.contains("Stream 2") || content2.contains("2"), "Second stream should mention Stream 2");
