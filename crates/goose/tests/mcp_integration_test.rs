...
//     assert!(
//         tool_names.iter().any(|name| name.contains("echo")),
//         "Echo tool not found in: {:?}",
//         tool_names
//     );
//     assert!(
//         tool_names.iter().any(|name| name.contains("add_numbers")),
//         "Add numbers tool not found in: {:?}",
//         tool_names
//     );
// }
...
//     };
//     assert!(
//         content_text.contains("Hello, MCP World!"),
//         "Echo response doesn't contain expected message: {}",
//         content_text
//     );
...
//     assert!(
//         content.is_ok(),
//         "Add numbers tool execution failed: {:?}",
//         content
//     );
...
//     };
//     assert!(
//         content_text.contains("8"),
//         "Add numbers response doesn't contain expected result: {}",
//         content_text
//     );
// }
...
//     assert!(
//         result.is_ok(),
//         "Tool dispatch should succeed even if tool execution fails"
//     );
...
//     assert!(
//         content.is_err(),
//         "Expected tool execution to fail for nonexistent tool"
//     );
...
async fn test_replayed_session() {
    ...
    let mode = TestMode::Replay;

    let extension_config = match mode {
        TestMode::Record => ExtensionConfig::Stdio {
            name: "test".to_string(),
            description: Some("Test".to_string()),
            cmd: "/Users/jackamadeo/development/goose/target/debug/stdio_logger".to_string(),
            args: vec![
                "/Users/jackamadeo/development/goose/test-mcp-log.log",
                "uv",
                "--project",
                "/Users/jackamadeo/development/stream-mcp",
                "run",
                "/Users/jackamadeo/development/stream-mcp/stream_mcp.py",
            ]
            .into_iter()
            .map(str::to_string)
            .collect(),
        ...
        TestMode::Replay => ExtensionConfig::Stdio {
            name: "test".to_string(),
            description: Some("Test".to_string()),
            cmd: "/Users/jackamadeo/development/goose/target/debug/stdio_replayer".to_string(),
            args: vec!["/Users/jackamadeo/development/goose/test-mcp-log.log".to_string()],
            envs: Envs::default(),
            env_keys: vec![],
            timeout: Some(30),
            bundled: Some(false),
    ...
    let tool_result = result.unwrap();
    let content = tool_result.result.await;
    assert!(content.is_ok(), "Expected tool execution to succeed");
}
