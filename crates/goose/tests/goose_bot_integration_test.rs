...
use goose::goose_bot::{
    ...
    GooseBotCommentRequest, GooseBotPrefs, ReviewSeverity, RoutingPrefs, TriggerPreference,
...
fn clear_install_removes_id_and_prefs() {
    ...
    assert_eq!(load_prefs_from(&config), GooseBotPrefs::default());
}
...
fn routing_default_matches_switchboard_fixture() {
    ...
    let expected =
        include_str!("../../../services/goose-bot-switchboard/fixtures/routing_prefs_default.json")
            .trim();
...
fn comment_request_is_pr_explicit_false() {
    let req: GooseBotCommentRequest = serde_json::from_str(
        r#"{
            "github_token": "t",
            "repo": "o/r",
            "pr_number": 1,
