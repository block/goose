...
│  Tokens:    Session JWT, OIDC token, X-Api-Key                      │
...
### 1. `X-Api-Key` Header Ambiguity

The `X-Api-Key` header is used for **user identity auth**, not provider authentication.

```
# This identifies the USER, not the LLM provider:
curl -H "X-Api-Key: my-key-123" https://goose-server/reply
...
**Don't confuse**: Sending an OpenAI API key as `X-Api-Key` will create a user identity
