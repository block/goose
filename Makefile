...
help:
	...
	@echo "  make dev-ui          Run Electron with local goose + avocado provider OAuth"
...
dev-ui:
	@chmod +x scripts/prepare-dev-ui-env.sh
	@$(MAKE) build-desktop-binary
	@GOOSE_SERVER__SECRET_KEY="$(GOOSE_SERVER__SECRET_KEY)" ./scripts/prepare-dev-ui-env.sh
	@echo "Desktop UI: $$(./scripts/with-node.sh 24 bash -c 'echo node $$(node -v), pnpm $$(pnpm -v)')"
	@echo "Local goose + avocado provider OAuth (Sign in during onboarding)."
