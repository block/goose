...
# Common Docker Windows build shell command, the inner script executed inside the rust:latest container
docker_windows_build_sh := """
...
release-windows:
    ...
    if [ "$(uname)" = "Darwin" ] || [ "$(uname)" = "Linux" ]; then
        ...
        docker run --rm \
            ...
            -w /usr/src/myapp \
            rust:latest \
            sh -c "{{docker_windows_build_sh}}"
    else
        echo "Building Windows executable using Docker through PowerShell..."
        powershell.exe -Command "Set-PSDebug -Trace 1; docker volume create goose-windows-cache; \`
            docker run --rm \`
            -v ${PWD}:/usr/src/myapp \`
            -v goose-windows-cache:/usr/local/cargo/registry \`
            -w /usr/src/myapp \`
