...
//
// Run with:
//   GOOSE_BINARY=/path/to/goose node ui/goose-acp-client/scripts/check-binary-compat.mjs
//
// Or via package script:
//   GOOSE_BINARY=/path/to/goose pnpm --filter @aaif/goose-acp-client run check:compat
...
if (!existsSync(CLIENT_DIST)) {
  console.error(
    `[compat] expected built client at ${CLIENT_DIST} — run pnpm build first`,
  );
  process.exit(1);
}
