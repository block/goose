...
# Publishes @block/goose and all native binary packages to npm.
#
# Usage:
#   ./ui/text/scripts/publish.sh         # publish all (dry-run)
#   ./ui/text/scripts/publish.sh --real   # publish for real
#
# Prerequisites:
#   - npm login to the @block scope
#   - Native binaries built via build-native-packages.sh
#   - ui/text built via `npm run build`
...
# Publish native binary packages first
