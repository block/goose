/**
 * Theme tokens for MCP Apps — the single source of truth.
 *
 ...
 *  1. Goose desktop — applied to :root via style.setProperty() per resolved theme.
 *  2. MCP apps — encoded as light-dark() in hostContext.styles.variables.
 *
 * App-specific aliases (sidebar, highlight, etc.) live in main.css as var()
 * references to these semantic tokens.
 */
import type {
  McpUiHostStyles,
 ...
 * Called on mount and whenever the resolved theme changes.
 */
