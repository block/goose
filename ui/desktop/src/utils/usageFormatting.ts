 ...
 * `precise` keeps the decimal at every magnitude (`12.3k`, `1.2M`) for headline figures.
 */
export function formatTokenCount(n: number, precise = false): string {
  if (n < 1000) {
    return Math.round(n).toString();
  }
  const [value, suffix] = n < 999_500 ? [n / 1000, 'k'] : [n / 1_000_000, 'M'];
  const text =
    precise || value < 10 ? value.toFixed(1).replace(/\.0$/, '') : Math.round(value).toString();
  return `${text}${suffix}`;
}
