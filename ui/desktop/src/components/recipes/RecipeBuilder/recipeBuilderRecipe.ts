...
const UI_STATE_KINDS = ['recipe_builder'] as const;
const RECIPE_BUILDER_KIND = 'recipe_builder';
...
2. **Generate a working recipe immediately** — call update_session_ui_state with kind "${RECIPE_BUILDER_KIND}" and the full recipe object as payload. Prefer action over perfection.
...
→ Generate a recipe right away from their description by calling update_session_ui_state with kind "${RECIPE_BUILDER_KIND}". Then refine.
...
→ Review it, suggest improvements, and call update_session_ui_state with kind "${RECIPE_BUILDER_KIND}" and the improved version as payload.
...
2. Call update_session_ui_state with kind "${RECIPE_BUILDER_KIND}" and the full recipe object as payload (not a partial diff)
...
Do not output recipe YAML. Use update_session_ui_state with kind "${RECIPE_BUILDER_KIND}" for any recipe you generate or update.
...
export const sessionUiStateExtension: ExtensionConfig = {
  type: 'frontend',
  name: 'session_ui_state',
  description: 'Update session UI state for a given kind.',
  tools: [
    {
      name: 'update_session_ui_state',
      description:
        'Update session UI state for a given kind. Use only when you have a complete, valid update.',
      inputSchema: {
        type: 'object',
        ...
        properties: {
          kind: {
            type: 'string',
            description: `UI state kind (kinds: ${UI_STATE_KINDS.join(', ')}).`,
          },
          payload: {
            type: 'object',
            description: 'State payload for the given kind.',
          },
        },
      },
    },
  ],
  instructions: `Use update_session_ui_state only when you have a complete, valid update. Do not call it during general conversation. For recipe updates, use kind "${RECIPE_BUILDER_KIND}" and payload set to the full recipe object.`,
};
