...
import { Attach, Close, Microphone, Send } from '@/components/atoms/icons';
import Stop from '@/components/atoms/stop';
import { DiagnosticsModal } from '@/components/molecules/ui/diagnostics';
import { BottomMenuAgentSelection } from '@/components/organisms/bottom-menu/BottomMenuAgentSelection';
import { BottomMenuExtensionSelection } from '@/components/organisms/bottom-menu/BottomMenuExtensionSelection';
import { BottomMenuModeSelection } from '@/components/organisms/bottom-menu/BottomMenuModeSelection';
import { CostTracker } from '@/components/organisms/bottom-menu/CostTracker';
import { DirSwitcher } from '@/components/organisms/bottom-menu/DirSwitcher';
import { AppEvents } from '@/constants/events';
import { useConfig } from '@/contexts/ConfigContext';
import { useModelAndProvider } from '@/contexts/ModelAndProviderContext';
import { useAudioRecorder } from '@/hooks/useAudioRecorder';
import { type DroppedFile, useFileDrop } from '@/hooks/useFileDrop';
...
export default function ChatInput({
    ...
    if ((totalTokens && totalTokens > 0) || (isTokenLimitLoaded && tokenLimit)) {
      addAlert({
        ...
        onCompact: () => {
          ...
          handleSubmitRef.current({ msg: MANUAL_COMPACT_TRIGGER, images: [] });
        },
    ...
    if (toolCount !== null && toolCount > TOOLS_MAX_SUGGESTED) {
      addAlert({
        ...
        action: {
          ...
          onClick: () => setViewRef.current('extensions'),
        },
  ...
  const getDictationUnavailableTooltip = (): string => {
    if (!dictationProvider) return '';

    if (dictationProvider === 'openai') {
      return 'OpenAI API key is not configured. Set it up in Settings > Models.';
    }

    if (dictationProvider === 'elevenlabs') {
      return 'ElevenLabs API key is not configured. Set it up in Settings > Chat > Voice Dictation.';
    }

    if (dictationProvider === 'local') {
      return 'Local Whisper model not found. Download a model in Settings > Dictation > Local (Offline).';
    }

    return 'Dictation provider is not properly configured.';
  };
  ...
  return (
    ...
    <div
      ...
      } bg-background-default z-10 rounded-3xl`}
      data-drop-zone="true"
      ...
      <form onSubmit={onFormSubmit} className="relative">
        <div className="relative">
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
            ...
            {dictationProvider &&
              (!isEnabled ? (
                <span className="inline-flex" title={getDictationUnavailableTooltip()}>
                  <Button
                    type="button"
                    size="sm"
                    shape="round"
                ...
                <Button
                  ...
                  title={isRecording ? 'Stop dictation' : 'Voice dictation • Say "submit" to send'}
            ...
            <>
              ...
              <div className="flex items-center h-full">
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  title={recipe ? 'View/Edit Recipe' : 'Create Recipe from Session'}
                  aria-label={recipe ? 'View or edit recipe' : 'Create recipe from session'}
