...
export interface ChatInputRef {
  ...
  canNavigateHistory: (direction: 'previous' | 'next') => boolean;
  submitPrompt: () => void;
  canSend: () => boolean;
}
...
const ChatInput = forwardRef<ChatInputRef, ChatInputProps>(
  (
    ...
    const navigateHistory = (direction: 'previous' | 'next') => {
      ...
      if (direction === 'previous') {
        if (newIndex < currentHistory.length - 1) {
          ...
          // Only switch to global history if it contains different prompts
          const hasUniqueGlobalPrompts = globalHistory.some(
            (prompt) => !commandHistory.includes(prompt)
          );
          if (hasUniqueGlobalPrompts) {
    ...
    const canNavigateHistoryDirection = (direction: 'previous' | 'next') => {
      const globalHistory = LocalMessageStorage.getRecentMessages() || [];
      ...
      if (historyIndex !== -1) {
        ...
        if (direction === 'previous') {
          ...
          // Can switch to global history only if it has unique prompts
          if (!isInGlobalHistory && globalHistory.length > 0) {
            const hasUniqueGlobalPrompts = globalHistory.some(
              (prompt) => !commandHistory.includes(prompt)
            );
            return hasUniqueGlobalPrompts;
          }
          return false;
        } else {
          return (
            historyIndex > 0 ||
            (isInGlobalHistory && commandHistory.length > 0) ||
            historyIndex >= 0
          );
        }
      } else {
        if (direction === 'previous') {
          return commandHistory.length > 0 || globalHistory.length > 0;
        } else {
          return false;
        }
      }
    ...
    const canSendPrompt = () => {
      ...
      return (
        !isLoading && hasSubmittableContent && !isAnyImageLoading && !isRecording && !isTranscribing
      );
    };

    useImperativeHandle(ref, () => ({
      navigateHistory,
      canNavigateHistory: canNavigateHistoryDirection,
      submitPrompt: performSubmit,
    ...
    }, [isLoading, pastedImages, isRecording, isTranscribing]);
    ...
    const handleHistoryNavigation = (evt: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      evt.preventDefault();
      navigateHistory(isUp ? 'previous' : 'next');
    };
