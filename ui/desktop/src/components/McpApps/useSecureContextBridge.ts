/**
 * Secure Context Bridge for MCP Apps
 *
 * This hook provides communication between the Host and an MCP App loaded
 * in an iframe with a real URL (not srcdoc). This gives the MCP App a
 * proper origin and secure context.
 *
 * Key differences from useSandboxBridge:
 * - No sandbox proxy layer - direct iframe communication
 * - HTML is served from /mcp-app-view/{token} endpoint
 * - The iframe has a real localhost origin (secure context)
 * - postMessage still used for JSON-RPC communication
 */
...
interface SecureContextBridgeOptions {
  resourceHtml: string;
...
interface SecureContextBridgeResult {
  iframeRef: React.RefObject<HTMLIFrameElement | null>;
  viewUrl: string | null;
  isLoading: boolean;
}

export function useSecureContextBridge(options: SecureContextBridgeOptions): SecureContextBridgeResult {
  const {
    resourceHtml,
    resourceCsp,
    resourcePermissions,
