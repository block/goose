...
function SplashPill({ content, append }) {
  return (
    <div
      className="px-16 py-8 text-14 text-center text-black/60 dark:text-white/60 whitespace-nowrap cursor-pointer bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 rounded-[1000px] inline-block transition-all duration-150"
      onClick={async () => {
        const message = {
          content,
          role: "user",
