...
const i18n = defineMessages({
  ...
  scheduleTriggered: {
    id: 'scheduleDetailView.scheduleTriggered',
  ...
  failedToLoadSession: {
    id: 'scheduleDetailView.failedToLoadSession',
  ...
  const handleRunNow = async () => {
    ...
    try {
      ...
      if (newSessionId === 'CANCELLED') {
        toastSuccess({
          title: intl.formatMessage(i18n.jobCancelled),
          msg: intl.formatMessage(i18n.jobCancelledMsg),
        });
      } else {
        toastSuccess({
          title: intl.formatMessage(i18n.scheduleTriggered),
          msg: intl.formatMessage(i18n.newSession, { sessionId: newSessionId }),
        });
      }
                      ...
                      session.accumulatedTotalTokens !== null && (
                        <p className="text-xs text-text-secondary mt-1">
                          {intl.formatMessage(i18n.tokens, {
                            count: session.accumulatedTotalTokens,
                          })}
                        </p>
                      )}
                    <p className="text-xs text-text-secondary mt-1">
