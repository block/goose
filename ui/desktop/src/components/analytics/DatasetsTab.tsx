    ...
    <div className="space-y-4">
      ...
      {yamlMode ? (
        <textarea
          ...
          rows={20}
          className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-green-300 text-xs font-mono focus:outline-none focus:border-blue-500"
        />
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-700/50">
                  ...
                  <th className="text-left px-3 py-2 text-gray-400 font-medium">Input Prompt</th>
                  <th className="text-left px-3 py-2 text-gray-400 font-medium w-40">Expected Agent</th>
                  <th className="text-left px-3 py-2 text-gray-400 font-medium w-40">Expected Mode</th>
                  <th className="text-left px-3 py-2 text-gray-400 font-medium w-36">Tags</th>
                  <th className="w-10" />
                </tr>
              </thead>
              <tbody>
          ...
          <button onClick={addRow} className="text-sm text-blue-400 hover:text-blue-300 flex items-center gap-1">
            + Add test case
          </button>
        </>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-white">Evaluation Datasets</h3>
        <button
      ...
      {loading ? (
        <div className="space-y-3 animate-pulse">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-16 rounded-lg bg-gray-700/50" />
          ))}
        </div>
      ) : datasets.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-48 text-gray-400">
          <p className="text-lg mb-2">No datasets yet</p>
          <p className="text-sm mb-4">Create a dataset to define test cases for evaluating routing accuracy</p>
          <button
            onClick={() => setEditing("new")}
            className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium transition-colors"
          >
        ...
        <div className="space-y-2">
          {datasets.map((ds) => (
            <div
              key={ds.id}
              className="rounded-lg border border-gray-600/40 bg-gray-800/50 p-4 flex items-center justify-between hover:border-gray-500/40 transition-colors"
            >
              <div className="flex-1 min-w-0">
                ...
                {ds.description && (
                  <p className="text-sm text-gray-400 mt-1 truncate">{ds.description}</p>
                )}
              ...
              <div className="flex gap-2 ml-4">
                <button
                  ...
                  className="px-3 py-1.5 rounded border border-gray-600 text-gray-300 text-xs hover:bg-gray-700 transition-colors"
                >
                  Edit
                </button>
                <button
                  onClick={() => handleDelete(ds.id)}
                  className="px-3 py-1.5 rounded border border-red-600/40 text-red-400 text-xs hover:bg-red-900/30 transition-colors"
                >
                  Delete
                </button>
              </div>
