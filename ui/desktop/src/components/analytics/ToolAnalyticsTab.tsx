      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-gray-400 border-b border-gray-700">
            ...
            <th className="text-right py-2 px-3 cursor-pointer hover:text-white" onClick={() => setSortBy("calls")}>
              Calls {sortBy === "calls" && "↓"}
            </th>
            <th className="text-right py-2 px-3 cursor-pointer hover:text-white" onClick={() => setSortBy("errors")}>
              Errors {sortBy === "errors" && "↓"}
            </th>
            <th className="text-right py-2 px-3 cursor-pointer hover:text-white" onClick={() => setSortBy("rate")}>
        ...
        <tbody>
          {sorted.map((tool) => (
            <tr key={tool.tool_name} className="border-b border-gray-800 hover:bg-gray-800/30">
              <td className="py-2 px-3 font-mono text-xs text-white">{shortenToolName(tool.tool_name)}</td>
              <td className="py-2 px-3 text-gray-400 text-xs">{tool.extension || "—"}</td>
              <td className="py-2 px-3 text-right text-white">{tool.call_count.toLocaleString()}</td>
              <td className="py-2 px-3 text-right">
                <span className={tool.error_count > 0 ? "text-red-400" : "text-gray-500"}>
                  {tool.error_count}
                </span>
              </td>
              <td className="py-2 px-3 text-right">
                  ...
                  <div className="w-16 h-1.5 bg-gray-700 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: tool.success_rate >= 0.95 ? SUCCESS_COLOR : tool.success_rate >= 0.8 ? "#f59e0b" : ERROR_COLOR,
                      }}
                    />
                  </div>
                  <span className={
                    tool.success_rate >= 0.95 ? "text-green-400" : tool.success_rate >= 0.8 ? "text-amber-400" : "text-red-400"
                  }>
      ...
      <ResponsiveContainer width={200} height={200}>
        <PieChart>
          ...
          <Tooltip
            ...
            formatter={(value) => [typeof value === "number" ? value.toLocaleString() : value, "Calls"]}
          />
        </PieChart>
      </ResponsiveContainer>
      <div className="flex-1 space-y-2">
        {extensions.map((ext, i) => (
          <div key={ext.extension} className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-300 flex-1">{ext.extension}</span>
            <span className="text-xs text-gray-400">{ext.total_calls} calls</span>
            <span className={`text-xs ${ext.success_rate >= 0.95 ? "text-green-400" : ext.success_rate >= 0.8 ? "text-amber-400" : "text-red-400"}`}>
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <AreaChart data={data}>
        ...
        />
        <Area type="monotone" dataKey="tool_calls" name="Tool Calls" stroke="#6366f1" fill="url(#toolCallsGrad)" />
        <Area type="monotone" dataKey="tool_errors" name="Errors" stroke="#f43f5e" fill="url(#toolErrorsGrad)" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {sessions.slice(0, 20).map((s) => (
            <tr key={s.session_id} className="border-b border-gray-800 hover:bg-gray-800/30">
              <td className="py-2 px-3 text-white text-xs truncate max-w-[200px]">{s.session_name || s.session_id.slice(0, 8)}</td>
              <td className="py-2 px-3 text-right text-white">{s.tool_calls}</td>
              <td className="py-2 px-3 text-right">
                <span className={s.tool_errors > 0 ? "text-red-400" : "text-gray-500"}>{s.tool_errors}</span>
              </td>
              <td className="py-2 px-3 text-right text-gray-300">{s.unique_tools}</td>
              <td className="py-2 px-3 text-gray-400 font-mono text-xs">{shortenToolName(s.most_used_tool)}</td>
              <td className="py-2 px-3 text-right text-gray-400 text-xs">{s.created_at.slice(0, 10)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function AgentPerformanceView({ data }: { data: AgentPerformanceMetrics }) {
  return (
    <div className="space-y-6">
      {/* Provider breakdown */}
      <div>
        <h4 className="text-sm font-medium text-gray-300 mb-3">Sessions by Provider</h4>
        <div className="grid grid-cols-2 gap-3">
          {data.sessions_by_provider.map((p) => (
            <div key={p.provider} className="bg-gray-800/30 rounded-lg p-3 border border-gray-700/30">
              <div className="text-xs text-gray-400">{p.provider}</div>
              <div className="text-lg font-bold text-white">{p.session_count} sessions</div>
              <div className="text-xs text-gray-500 mt-1">
                Avg {Math.round(p.avg_tokens).toLocaleString()} tokens · {p.avg_messages.toFixed(1)} msgs/session
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-300 mb-3">Session Duration</h4>
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-gray-800/30 rounded-lg p-3 border border-gray-700/30 text-center">
            <div className="text-xs text-gray-400">Average</div>
            <div className="text-lg font-bold text-white">{formatDuration(data.session_duration_stats.avg_seconds)}</div>
          </div>
          <div className="bg-gray-800/30 rounded-lg p-3 border border-gray-700/30 text-center">
            <div className="text-xs text-gray-400">Median</div>
            <div className="text-lg font-bold text-white">{formatDuration(data.session_duration_stats.median_seconds)}</div>
          </div>
          <div className="bg-gray-800/30 rounded-lg p-3 border border-gray-700/30 text-center">
            <div className="text-xs text-gray-400">P90</div>
            <div className="text-lg font-bold text-white">{formatDuration(data.session_duration_stats.p90_seconds)}</div>
          </div>
        </div>
      </div>
      ...
      {data.active_extensions.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-gray-300 mb-3">Active Extensions</h4>
          <div className="flex flex-wrap gap-2">
            {data.active_extensions.map((ext) => (
              <span key={ext.extension} className="px-2.5 py-1 text-xs rounded-full bg-indigo-900/30 text-indigo-300 border border-indigo-700/30">
...
function LoadingSkeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="h-[220px] bg-gray-800/50 rounded-xl" />
      <div className="h-[300px] bg-gray-800/50 rounded-xl" />
    </div>
  );
}
...
export default function ToolAnalyticsTab() {
  ...
  if (!toolData) return <div className="text-gray-400 text-center py-8">No tool data available</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          color={toolData.success_rate >= 0.95 ? "text-green-400" : toolData.success_rate >= 0.8 ? "text-amber-400" : "text-red-400"}
      ...
      <div className="flex gap-1 border-b border-gray-700 pb-0">
        {(["tools", "extensions", "sessions", "agents"] as const).map((tab) => (
          ...
          >
            {tab === "tools" ? "Tool Usage" : tab === "extensions" ? "Extensions" : tab === "sessions" ? "Sessions" : "Agent Performance"}
          </button>
        ))}
      </div>
      ...
      {toolData.daily_tool_activity.length > 0 && (
        <div className="bg-gray-800/30 rounded-xl p-4 border border-gray-700/30">
          <h3 className="text-sm font-medium text-gray-300 mb-3">Daily Tool Activity</h3>
      ...
      {view === "extensions" && (
        <div className="bg-gray-800/30 rounded-xl p-4 border border-gray-700/30">
          <h3 className="text-sm font-medium text-gray-300 mb-3">Extension Breakdown</h3>
          {toolData.extension_usage.length > 0 ? (
            <ExtensionBreakdown extensions={toolData.extension_usage} />
          ) : (
            <div className="text-gray-500 text-center py-8">No extension data available</div>
          )}
        </div>
      )}
      ...
      {view === "agents" && agentData && (
        <div className="bg-gray-800/30 rounded-xl p-4 border border-gray-700/30">
          <h3 className="text-sm font-medium text-gray-300 mb-3">Agent Performance</h3>
