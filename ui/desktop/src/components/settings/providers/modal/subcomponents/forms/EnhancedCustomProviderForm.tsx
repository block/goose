...
export default function EnhancedCustomProviderForm({
  ...
  useEffect(() => {
    if (template) {
      ...
      setSelectedModels(new Set(initialData.models));
  ...
  const selectAllModels = () => {
    if (!template) return;
    setSelectedModels(new Set(template.models.map((m) => m.id)));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedModels.size === 0) errors.models = 'At least one model must be selected';
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 space-y-4">
      ...
      {/* Display Name - read-only if from template */}
      <div>
        <label
      ...
      {/* API URL - read-only if from template */}
      <div>
        <label
          htmlFor="api-url"
          className="flex items-center text-sm font-medium text-textStandard mb-2"
      ...
      {template && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="flex items-center text-sm font-medium text-textStandard">
              Available Models
              <span className="text-red-500 ml-1">*</span>
            </label>
