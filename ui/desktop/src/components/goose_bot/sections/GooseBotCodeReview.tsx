...
import { useGooseBotRepos } from '../useGooseBotRepos';
import { RecipeModelSelector } from '../../recipes/shared/RecipeModelSelector';

const i18n = defineMessages({
  setupBlurb: {
    id: 'gooseBotCodeReview.setupBlurb',
    defaultMessage:
      'When code review is on, Goose Bot automatically reviews pull requests and leaves inline suggestions. Mention @aaif-goose-bot in any PR comment to ask a question, request a manual review, or apply fixes.',
  },
  reviewTitle: {
    id: 'gooseBotCodeReview.reviewTitle',
    defaultMessage: 'Code review',
  },
  reviewDescription: {
    id: 'gooseBotCodeReview.reviewDescription',
    defaultMessage: 'Configure when and how Goose Bot reviews pull requests.',
  },
  autoReviewLabel: {
    id: 'gooseBotCodeReview.autoReviewLabel',
    defaultMessage: 'Personal auto review preferences',
  },
  autoReviewHelper: {
    id: 'gooseBotCodeReview.autoReviewHelper',
    defaultMessage:
      'All your pull requests in a Goose Bot-enabled repository will be automatically reviewed.',
  },
  triggerLabel: {
    id: 'gooseBotCodeReview.triggerLabel',
    defaultMessage: 'Personal review trigger preference',
  },
  triggerHelper: {
    id: 'gooseBotCodeReview.triggerHelper',
    defaultMessage: 'Choose when Goose Bot should automatically review your pull requests.',
  },
  outputStyleLabel: {
    id: 'gooseBotCodeReview.outputStyleLabel',
    defaultMessage: 'Review output style',
  },
  outputStyleHelper: {
    id: 'gooseBotCodeReview.outputStyleHelper',
    defaultMessage: 'How findings are posted back to the pull request.',
  },
  severityLabel: {
    id: 'gooseBotCodeReview.severityLabel',
    defaultMessage: 'Minimum severity to surface',
  },
  severityHelper: {
    id: 'gooseBotCodeReview.severityHelper',
    defaultMessage:
      'Findings below this rank are dropped from the review. Higher = quieter.',
  },
  reviewModelLabel: {
    id: 'gooseBotCodeReview.reviewModelLabel',
    defaultMessage: 'Review model',
  },
  reviewModelHelper: {
    id: 'gooseBotCodeReview.reviewModelHelper',
    defaultMessage:
      'Which model the review orchestrator uses. Defaults to the model goose is configured with.',
  },
  permissionsTitle: {
    id: 'gooseBotCodeReview.permissionsTitle',
    defaultMessage: 'Permissions',
  },
  permissionsDescription: {
    id: 'gooseBotCodeReview.permissionsDescription',
    defaultMessage: 'Control who can interact with @aaif-goose-bot and what it is allowed to do.',
  },
  triggerPermissionLabel: {
    id: 'gooseBotCodeReview.triggerPermissionLabel',
    defaultMessage: 'Who can trigger Goose Bot',
  },
  triggerPermissionHelper: {
    id: 'gooseBotCodeReview.triggerPermissionHelper',
    defaultMessage:
      'Restrict who can @mention the bot to request reviews, fixes, or replies.',
  },
  allowCommitLabel: {
    id: 'gooseBotCodeReview.allowCommitLabel',
    defaultMessage: 'Allow pushing commits to PR branches',
  },
  allowCommitHelper: {
    id: 'gooseBotCodeReview.allowCommitHelper',
    defaultMessage:
      'When someone asks @aaif-goose-bot to fix issues, push the resulting changes to the PR branch as aaif-goose-bot[bot].',
  },
  allowIssuesLabel: {
    id: 'gooseBotCodeReview.allowIssuesLabel',
    defaultMessage: 'Respond to mentions on issues',
  },
  allowIssuesHelper: {
    id: 'gooseBotCodeReview.allowIssuesHelper',
    defaultMessage:
      'Let @aaif-goose-bot answer questions and draft PRs from issue comments, not just from inside pull requests.',
  },
  allowNewPrsLabel: {
    id: 'gooseBotCodeReview.allowNewPrsLabel',
    defaultMessage: 'Open new pull requests for issue fixes',
  },
  allowNewPrsHelper: {
    id: 'gooseBotCodeReview.allowNewPrsHelper',
    defaultMessage:
...
interface Props {
  ...
  onUpdate: (patch: Partial<GooseBotPrefs>) => void;
}

export default function GooseBotCodeReview({ prefs, onUpdate }: Props) {
  const intl = useIntl();
  const [repoQuery, setRepoQuery] = useState('');
  const [hideArchivedRepos, setHideArchivedRepos] = useState(true);
  const { state: reposState, refresh: refreshRepos } = useGooseBotRepos(prefs !== null);
  ...
  const triggerOptions = useMemo<RichOption<TriggerPreference>[]>(
    () => [
      ...
      },
      {
        value: 'manual-only',
        label: 'Manual only',
        description: 'Only review when someone mentions @aaif-goose-bot review.',
      },
    ],
    []
  );
    ...
    () => [
      {
        ...
        label: 'Anyone',
        description:
          'Any GitHub user who can see the PR can mention @aaif-goose-bot. Best for open-source.',
      },
      {
        value: 'write-access',
...
function RepoTable({
  ...
  state: ReturnType<typeof useGooseBotRepos>['state'];
  query: string;
...
function RepoRow({ repo }: { repo: GooseBotRepo }) {
  return (
