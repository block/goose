...
app.whenReady().then(async () => {
  ...
  async function runRepoIndexForWindow(
    ...
    try {
      ...
      const winConfig = winConfigStr ? JSON.parse(winConfigStr) : {};
      const cwd: string =
      ...
      const goosePath = getBinaryPath(app, 'goose');
      ...
      const result: Promise<{ ok: boolean; outputPath?: string; error?: string }> = new Promise(
        (resolve) => {
          child.on('close', (code) => {
            ...
            else resolve({ ok: false, error: `goose exited with code ${code}` });
          });
