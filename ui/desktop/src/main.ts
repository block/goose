...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('launch-goose-app', async (_, appName: string, jsImplementation: string) => {
    ...
    try {
      ...
      // Create the HTML content with the app injected
      const htmlContent = `
<!doctype html>
<html>
  <head>
    <meta charset="UTF-8"/>
    <title>Goose App - ${appName}</title>
    <style>
      body {
        margin: 0;
        padding: 0;
  ...
  <body>
    ...
    <script>
      // Goose App API
      window.gooseApp = {
        // Basic lifecycle and rendering
        render: function(elementId) {
          console.warn('render() method not implemented by app');
        },
        ...
        storage: {
          get: function(key) {
            try {
              return JSON.parse(localStorage.getItem('goose-app-' + key));
            } catch (e) {
              return null;
            }
          },
          set: function(key, value) {
            try {
              localStorage.setItem('goose-app-' + key, JSON.stringify(value));
              return true;
            } catch (e) {
              return false;
            }
          ...
          remove: function(key) {
            localStorage.removeItem('goose-app-' + key);
          }
        },
      ...
      try {
        ...
        function initializeApp() {
          if (typeof window.gooseAppManifest === 'object' && window.gooseAppManifest) {
            console.log('Goose App loaded:', window.gooseAppManifest.name);
            
            // Call app's onLoad if it exists
            if (typeof window.gooseApp.onLoad === 'function') {
              window.gooseApp.onLoad();
            }
            
            // Call app's render method
            if (typeof window.gooseApp.render === 'function') {
              window.gooseApp.render('app-container');
            }
            
            // Handle window resize
            window.addEventListener('resize', function() {
              if (typeof window.gooseApp.onResize === 'function') {
                window.gooseApp.onResize(window.innerWidth, window.innerHeight);
              }
            });
            
            // Handle window unload
            window.addEventListener('beforeunload', function() {
              if (typeof window.gooseApp.onUnload === 'function') {
                window.gooseApp.onUnload();
              }
            });
          } else {
            console.error('Invalid Goose App: Missing manifest');
            document.body.innerHTML = '<div style="padding: 20px; text-align: center;"><h2>Error</h2><p>Invalid Goose App: Missing manifest</p></div>';
          }
        }
        
      } catch (error) {
        console.error('Error loading Goose App:', error);
        document.body.innerHTML = '<div style="padding: 20px; text-align: center;"><h2>Error</h2><p>Failed to load Goose App: ' + error.message + '</p></div>';
      }
    </script>
  </body>
</html>`;
      ...
      appWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(htmlContent)}`);
      ...
      console.error('Error launching goose app:', error);
