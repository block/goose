...
export function SessionStreamProvider({ children }: { children: ReactNode }) {
  ...
  const updateSessionState = (sessionId: string, updates: Partial<SessionStreamState>) => {
  ...
  const getSessionState = (sessionId: string): SessionStreamState => {
    return (
  ...
  const value: SessionStreamContextValue = {
    getSessionState,
    refreshSession,
  };
  ...
  return (
    <SessionStreamContext.Provider value={value}>
      <SessionStreamManager startStream={startStream} stopStream={stopStream}>
        {children}
...
/**
 * Internal component that manages stream lifecycle based on active hooks
 */
function SessionStreamManager({
  children,
  startStream,
  stopStream,
}: {
  ...
  // Expose methods to register/unregister sessions
  useEffect(() => {
    ...
    const manager: SessionStreamManager = {
    ...
    (
      window as typeof window & { __sessionStreamManager?: SessionStreamManager }
    ).__sessionStreamManager = manager;

    return () => {
      delete (window as typeof window & { __sessionStreamManager?: SessionStreamManager })
