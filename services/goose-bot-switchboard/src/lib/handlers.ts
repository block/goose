...
const MENTION_RE = /(^|\s)@aaif-goose-bot\b/i;
const REVIEW_TRIGGER_RE = /(^|\s)@aaif-goose-bot\s+review\s*$/im;
...
// /register once the user enables Goose Bot. We only need to clean up KV on uninstall.
...
export async function handleIssueComment(payload: IssueCommentEvent, env: Env): Promise<void> {
  ...
  // Bot accounts trigger nothing - prevents aaif-goose-bot[bot] from replying
...
async function triggerComment(opts: {
  ...
  if (!install) {
    ...
    await postIssueComment(
      fullName,
      prNumber,
      `@${commenter} Goose Bot is installed on this repo but no local goose is connected. ` +
        `Open Goose Desktop > Code Review > Connectors to enable.`,
      token
    ).catch((err) =>
  ...
  try {
    ...
    try {
      await postIssueComment(
        fullName,
        prNumber,
        `@${commenter} Goose Bot couldn't reach your local goose. ` +
...
async function triggerReview(opts: {
  ...
  if (!install) {
    await postNeutralCheck({
      ...
      installationId,
      env,
      title: 'Goose Bot not enabled',
      summary:
        'No local goosed is registered for this installation. Open Goose Desktop > Code Review > Connectors to start receiving reviews.',
      token: opts.token,
    }).catch((err) =>
      console.error(
    ...
    if (checkRunId !== undefined) {
      try {
        await completeCheckRun({
          ...
          checkRunId,
          conclusion: 'neutral',
          title: 'Goose Bot offline',
          summary: `Could not reach your local goosed via the tunnel:\n\n${msg}\n\nMake sure Goose Desktop is running and try \`@aaif-goose-bot review\` to retry.`,
          token,
        });
      } catch (completeErr) {
        console.error(
