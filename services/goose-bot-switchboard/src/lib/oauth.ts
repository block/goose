const UA = 'goose-bot-switchboard/0.1';
...
export async function exchangeCodeAndResolve(opts: {
  ...
  if (mine.length === 0) {
    return {
      ...
      status: 409,
      error:
        'Goose Bot is not installed on any account this user can access. Install the GitHub App first.',
    };
  }
  if (mine.length > 1) {
    const accounts = mine
    ...
    return {
      ok: false,
      status: 409,
      error: `Goose Bot is installed on multiple accounts (${accounts.join(', ')}). Install it on one account or reconnect after narrowing installation access.`,
    };
  }
  const chosen = mine[0];
  return {
