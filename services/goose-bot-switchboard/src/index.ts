...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (request.method === 'GET' && url.pathname === '/') {
      return new Response('Goose Bot switchboard\n', {
        status: 200,
    ...
    if (request.method === 'POST' && url.pathname === '/goose-bot/register') {
      return handleRegisterRoute(request, env);
    }

    if (request.method === 'PUT' && url.pathname === '/goose-bot/routing-prefs') {
      return handleRoutingPrefs(request, env);
    }

    if (request.method === 'POST' && url.pathname === '/goose-bot/whoami') {
      return handleWhoami(request, env);
    }

    if (request.method === 'DELETE' && url.pathname === '/goose-bot/install') {
      return handleUnregister(request, env);
    }

    if (request.method === 'GET' && url.pathname === '/goose-bot/repos') {
      return handleListRepos(request, env);
    }

    if (request.method === 'GET' && url.pathname === '/goose-bot/analytics') {
      return handleAnalyticsGet(request, env);
    }

    if (request.method === 'POST' && url.pathname === '/goose-bot/analytics/event') {
      return handleAnalyticsEvent(request, env);
    }

    if (request.method === 'GET' && url.pathname === '/goose-bot/oauth-config') {
      return new Response(
