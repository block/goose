...
docker pull ghcr.io/aaif/goose:latest

# Run goose CLI
docker run --rm ghcr.io/aaif/goose:latest --version
...
docker run --rm \
  ...
  ghcr.io/aaif/goose:latest run -t "Hello, world!"
```
...
```bash
git clone https://github.com/aaif/goose.git
cd goose
```
...
services:
  goose:
    image: ghcr.io/aaif/goose:latest
    environment:
...
FROM ghcr.io/aaif/goose:latest
USER root
...
```dockerfile
FROM ghcr.io/aaif/goose:v1.6.0
...
- [Installation Guide](https://aaif.github.io/goose/docs/getting-started/installation) - All installation methods
- [Configuration Guide](https://aaif.github.io/goose/docs/guides/config-files) - Detailed configuration options
